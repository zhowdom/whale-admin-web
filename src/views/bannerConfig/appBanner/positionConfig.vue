  ...
  <div class="tab-content">
    ...
    <div class="task-table">
      ...
      style="width: 100%">
        ...
        <el-table-column prop="createTime" label="更新时间" align="center" min-width="160" :key="Math.random()">
          <template slot-scope="{row}">
...
<script>
import { bannerConfigAppPage, changeBannerPosition, bannerConfigAppSelect } from "@/api/bannerConfig";
export default {
  components: {},
  props: {},
  data () {
    ...
    bannerConfigAppSelect().then(response => {
      if (response.code == 200) {
        response.data.list.forEach(v => {
          let {code, desc, ...a} = v
          this.bannerConfigAppSelectObj[code] = desc
        })
      }
    })
  },
  ...
  methods: {
    ...
    search () {
      this.loading = true
      bannerConfigAppPage({}).then(response => {
        if (response.code === 200) {
