    ...
    <el-dialog custom-class="certifyForm" :title="dialogTitle" :visible.sync="dialogFormVisible" :width="'630px'">
      <el-form :model="certifyform" :rules="rules" ref="ruleForm" :disabled="seeBool">
        <el-form-item label="banner位置：" :label-width="formLabelWidth" prop="position">
          <el-select v-model="certifyform.position" clearable placeholder="请选择" class="mb20" @change="handlePositionChange">
        ...
        <el-form-item label="业务类型：" :label-width="formLabelWidth" prop="bizType">
          <el-select v-model="certifyform.bizType" clearable placeholder="请选择" class="mb20">
            <el-option v-for="(item, index) of coinServiceArr" :key="index" :label="item.desc" :value="item.code" :disabled="item.disable"></el-option>
          </el-select>
        </el-form-item>
...
import { bannerConfigPage, addBannerConfig, editBannerConfig, changeBannerConfig, deleteBannerConfig, bizTypeList, bannerConfigDappSelect } from "@/api/bannerConfig";
export default {
  props: {
  ...
  methods: {
    adsRent_addressList() {
        bannerConfigDappSelect({}).then(response => {
    ...
    handleEdit(row, seeBool = false) {
      ...
      this.handlePositionChange(row.position, true);
    ...
    // 选择位置查询下拉业务类型
    handlePositionChange(val, isBool) {
      if(!isBool) {
        this.certifyform.bizType = '';
      }
      bizTypeList({position: val}).then(response => {
        if (response.code === 200) {
