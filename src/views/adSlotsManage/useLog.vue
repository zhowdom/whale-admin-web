<template>
  <div class="userlistwapper" style="padding:20px;">
    ...
    <el-button type="primary" v-hasPermi="['ad_space:use_record:page']" style="margin-left:10px;" @click="search(1)">查 询</el-button>
    </el-form>
    ...
    <el-table highlight-current-row
      ...
      <!-- <el-table-column
        header-align="center"
        align="center"
        prop="useAdPlaceQuantity"
        label="每份套餐次数（次）">
      </el-table-column> -->
      <el-table-column
        header-align="center"
        align="center"
        prop="singleAdPlaceTime"
        label="每份套餐时长（时）">
      </el-table-column>
      <!-- <el-table-column
        header-align="center"
        align="center"
        prop="remainingAdPlaceQuantity"
        label="剩余次数">
      </el-table-column> -->
      <el-table-column
        header-align="center"
        align="center"
        prop="remainingAdPlaceTime"
        label="剩余时长">
      </el-table-column>
...
<script>
import {adsRent_useLog, adsRent_packageList, adsRent_switch, adsRent_addressList} from '@/api/moreCoin/index';
...
export default {
  ...
  methods: {
    adsRent_addressListRun () {
      return new Promise(resolve => {
        adsRent_addressList().then(response => {
          ...
          this.formAds.sizeScale = response?.data?.list[0]?.picType == 1 ? '13/3' : '13/5' // picType:1小banner；3:大banner
    ...
    search (pgi) {
      ...
      this.loading = true
      adsRent_useLog(_params).then(response => {
        if (response.code === 200) {
