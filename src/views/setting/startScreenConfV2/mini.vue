    ...
    <el-card class="wrap-card" shadow="always">
      <el-form :model="mainForm" :rules="rules" ref="mainForm" label-width="150px" class="custom-form">
        ...
        <el-form-item label="渠道来源：" prop="linkType">
          ...
          <el-form-item label="参数名-参数值：" v-if="+mainForm.linkType === 1">
          ...
          <el-form-item label="参数名-参数值：" v-if="+mainForm.linkType === 2">
            <ul style="margin:0;padding:0;">
  ...
  methods: {
    ...
    jumpLinkArrQuery () {
      return new Promise((resolve, reject) => {
        // linkType: 1外链2内链
        setRoutLinkList({linkType: 1, appPackageType: 3 }).then(response => {
        ...
        setRoutLinkList({linkType: 2, appPackageType: 3 }).then(response => {
          if (+response.code === 200) {
