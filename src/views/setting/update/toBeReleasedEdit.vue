        ...
        <el-form :model="updateForm" :rules="rules" ref="updateForm" label-width="200px" class="mt50">
          ...
          <el-form-item class="package-item" label="填写下载地址或上传安装包" prop="upgradeLink">
            ...
            <template v-if="updateForm.upgradeLink">
              <el-alert class="input-url" v-if="updateForm.platform==3" type="info" :closable="false" :title="updateForm.upgradeLink"></el-alert>
              <el-alert class="input-url" v-if="updateForm.platform!=3" @close="handleRemove(null, [])" type="info" :title="updateForm.upgradeLink"></el-alert>
            </template>
            </el-alert>
          </el-form-item>
...
export default {
  ...
  mounted () {
    ...
    if (this.isEdit) {
      ...
      this.versionInfoRequest();
    } else {
      this.updateForm.platform = 3;
  ...
  methods: {
    ...
    async getHistoryLatest (platform) {
      try {
        ...
        const defaultUrl = this.updateForm.platform == 3 ? this.iosUrl : this.updateForm.upgradeLink;
        this.updateForm.upgradeLink = defaultUrl;
        // 平台切换时清空选中的市场
        this.updateForm.auditMarketTypeList = [];
        const {code, message, data} = await historyVersion(
          Object.assign({platform}, {applicationPackageType: 1}) // old app 1 new app 2
        );
        if (code == 200) {
          if (data.length) {
    ...
    async versionInfoRequest () {
      try {
        ...
        if (code == 200) {
          // 获取历史最新版本
          await this.getHistoryLatest(data.platform);
          // 获取匹配的历史版本
          await this.historyVersionRequest({
            platform: data.platform,
            versionNo: data.compatibleVersion,
          });

          this.updateForm = {...this.updateForm, ...data};
          this.uploadConf.fileList = [{name: '', url: data.upgradeLink}];
          this.compatibleVersion = data.compatibleVersion;
          
          let templArr = [];
          let auditMarketTypeList = data.auditMarketTypeList || [];
          auditMarketTypeList.forEach(val => {
            if (data.platform == 3 && (val == 1 || val == 2)) {
              templArr.push(val);
            } else if (data.platform == 2 && val != 1 && val != 2) {
              templArr.push(val);
            }
          });
          // 过滤不同平台的市场
          this.updateForm.auditMarketTypeList = templArr;
        } else {
          this.$message({type: 'error', message});
        }
      } catch (error) {
    ...
    async historyVersionRequest (reqData) {
      try {
        this.updateForm.compatibleVersion = this.compatibleVersion;
        const {code, message, data} = await historyVersion(
          Object.assign(reqData, {applicationPackageType: 1}) // old app 1 new app 2
        );
        if (code == 200) {
          this.history = data;
          this.showSelectLoading = false;
          // !data.length && (this.updateForm.compatibleVersion = '');
        } else {
          this.$message({type: 'error', message});
        }
      } catch (error) {
    ...
    closeTagFn () {
      ...
      this.$store.dispatch('tagsView/delView', curView).then(() => {
        this.$nextTick(() => {
          this.$router.replace({path: 'versionUpdate'});
