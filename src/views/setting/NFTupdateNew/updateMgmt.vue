<template>
  <div class="update-wrap">
      ...
      <el-table border :data="tableData" :default-sort="defaultTabOrder" style="width: 100%" @sort-change="sortFunction">
        ...
        <el-table-column align="center" label="操作" width="290">
          <template slot-scope="{row}">
            ...
            <el-button size="mini" type="primary" v-hasPermi="['setting:version:update']" @click="handleEditBtn(row)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog
      title="更换下载链接"
      :visible.sync="dialogVisibleAlterLink"
      width="30%">
      <div class="clearfix" style="margin-bottom:20px;">
        <span class="fl" style="margin-right:10px;">下载链接:</span>
        <el-input clear="fl" v-model="inputLink" type="textarea" :rows="4" placeholder="请输入" style="width:80%"></el-input>
      </div>
      <div class="clearfix">
        <span class="fl" style="margin-right:10px;">版本描述:</span>
        <el-input clear="fl" v-model="inputMemo" type="textarea" :rows="3" placeholder="请输入" style="width:80%"></el-input>
      </div>
      <div class="clearfix">
        <span class="fl" style="margin-right:10px;">版本英文描述:</span>
        <el-input clear="fl" v-model="eninputMemo" type="textarea" :rows="3" placeholder="请输入" style="width:80%"></el-input>
      </div>

      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisibleAlterLink = false">取 消</el-button>
        <el-button type="primary" @click="handleEditBtnSure" :loading="dialogVisibleAlterLoading">确 定</el-button>
      </span>
    </el-dialog>
...
import {NFTversionPage as versionPage, NFTversionEdit as versionEdit, NFTlistVersions as listVersions, NFTauditMarket as auditMarket, NFTreleasesHistory as releasesHistory, NFTversionRecall as versionRecall} from '@/api/setting/version';

export default {
  name: 'UpdateMgmt',
  data () {
    return {
      dialogVisibleAlterLoading: false,
      curRowId: null,
      dialogVisibleAlterLink: false,
      inputLink: '',
      inputMemo: '',
      eninputMemo: '',
      platforms,
  ...
  methods: {
    ...
    onAdd () {
      ...
      this.$store.dispatch('tagsView/delView', curView).then(() => {
        this.$nextTick(() => {
          // 跳转页面到新增
          this.$router.replace({path: `NFTupdateAddNew`});
        })
      });
    },
    // 分页数量修改
    ...
    handleEditBtn (row) {
      // versionEdit
      this.inputLink = row.upgradeLink
      this.inputMemo = row.description
      this.eninputMemo = row.enDescription
      this.dialogVisibleAlterLink = true
      this.curRowId = row.id
    },
    checkUrl (url) {
      const re = /(http|https):\/\/\S*/
      return re.test(url)
    },
    async handleEditBtnSure () {
      if (!this.checkUrl(this.inputLink)) {
        this.$message.error('请输入正确的下载链接')
        return false
      }
      this.dialogVisibleAlterLoading = true
      const {code, message, data: {result}} = await versionEdit({
        description: this.inputMemo,
        enDescription: this.eninputMemo,
        id: this.curRowId,
        upgradeLink: this.inputLink
      });
      this.dialogVisibleAlterLoading = false
      if (code == 200) {
        this.$message.success(message);
        this.versionPageRequest();
        this.dialogVisibleAlterLink = false
      } else {
        this.$message.error(message);
      }
    },
