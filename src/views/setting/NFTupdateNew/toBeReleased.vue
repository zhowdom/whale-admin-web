<template>
  <div class="tobe-released-wrap">
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-form ref="versionForm" inline :model="versionForm">
            ...
            <el-form-item label="操作时间" prop="update">
              <el-date-picker
                class="align-width"
                v-model="update"
                type="daterange"
    ...
    <div class="update-table">
      <el-table border :data="tableData" style="width: 100%" :default-sort="defaultTabOrder" @sort-change="sortFunction">
        ...
        <el-table-column prop="updatedAt" sortable="custom" align="center" label="操作时间" width="170" :formatter="formatter"></el-table-column>
        <el-table-column align="center" label="操作" width="170">
          <template slot-scope="{row}">
            <el-button size="mini" type="primary" v-hasPermi="['setting:version:save']" @click="handleTabBtn(row, 'Edit')">编辑</el-button>
            <el-button size="mini" type="warning" v-hasPermi="['setting:version:released']" @click="handleRelease(row)">发布</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import {NFTversionPage as versionPage, NFTversionReleased as versionReleased, NFTlistVersions as listVersions, NFTauditMarket as auditMarket} from '@/api/setting/version';
...
export default {
  ...
  methods: {
    ...
    // 应用发布请求
    async versionReleasedRequest ({executor, executorId, platform, versionNo}) {
      try {
        const canBeRelease = executor && executorId && platform && versionNo;
        if (!canBeRelease) return this.$message.error('信息缺失无法发布应用！');

        const {code, message} = await versionReleased({
          executor, executorId, platform, versionNo,
          applicationPackageType: 2 // old app 1 new app 2
        });
        if (code == 200) {
    ...
    async versionPageRequest () {
      try {
        const [startUpdatedAt, endUpdatedAt] = this.update || [];

        this.versionForm.startUpdatedAt = startUpdatedAt;
        this.versionForm.endUpdatedAt = endUpdatedAt;
    ...
    onAdd () {
      ...
      this.$store.dispatch('tagsView/delView', curView).then(() => {
        this.$nextTick(() => {
          // 跳转页面到新增
          this.$router.replace({path: `NFTtoBeReleasedAddNew`});
        })
      });
    },
    // 分页数量修改
