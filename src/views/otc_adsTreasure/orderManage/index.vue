  ...
  <div class="userpower_Wrapper">
        <div class="topshow">
            ...
            <el-form-item label="用户邮箱" prop="buyerEmail" >
                <el-input clearable v-model="queryParams.buyerEmail" placeholder="请输入手机号"></el-input>
            </el-form-item>
            ...
            <el-form-item label="广告主邮箱" prop="sellerEmail" >
                <el-input clearable v-model="queryParams.sellerEmail" placeholder="请输入手机号"></el-input>
            </el-form-item>
          ...
          <el-form-item label="下单时间" prop="jointimeSelected">
            ...
            <el-form-item label="" style="border:none;">
                <el-button v-hasPermi="['otc:order:page']" :loading="loading" type="primary" icon="el-icon-search" @click="search(1)">查询</el-button>
            </el-form-item>
            </el-form>
        </div>
        ...
        style="width: 100%">
            ...
            <el-table-column
                prop="releaseProcessFee"
                align="center"
                maxlength="11"
                label="广告商手续费">
            </el-table-column>
            <el-table-column
                prop="buySaleProcessFee"
                align="center"
                maxlength="11"
                label="用户手续费">
            </el-table-column>
            ...
            <el-table-column
                ...
                label="操作">
                <template scope="scope">
                  <el-button type="text" @click="w2seeinClick(scope.row.id)">查看</el-button>
                  <el-button v-hasPermi="['otc:order:audit']" v-if="+scope.row.appealStatus === 2" type="text" @click="w2AuditClick(scope.row.id)">审核</el-button>
                  <el-button v-hasPermi="['otc:order:cancel']" v-if="+scope.row.orderStatus == 1 || (+scope.row.orderStatus == 2 && +scope.row.appealStatus !== 2)" type="text" @click="orderCancelClick(scope.row.id)">取消订单</el-button>
                </template>
            </el-table-column>
        ...
        <div class="mglls">
          ...
          <div>
            ...
            <p><span>OTC名称：</span><span>{{seeinMsg.name}}</span></p>
            <p><span>交易方向：</span><span>{{+seeinMsg.tickType === 2 ? '买入' : '卖出'}}</span></p>
            <p><span>交易数量：</span><span>{{seeinMsg.itemNum}}</span>个</p>
            <p><span>交易单价：</span><span>{{seeinMsg.salePrice}}</span>¥</p>
            <p><span>交易总价：</span><span>{{seeinMsg.orderAmount}}</span>¥</p>
            ...
            <!-- <p><span>OTC来源：</span><span>{{+seeinMsg.sourceType === 0 ? '自行兑换' : '市场买入'}}</span></p> -->
            <!-- <p><span>兑换单号：</span><span>{{seeinMsg.exchangeCode}}</span></p> -->
            <p><span>广告商手续费：</span><span>{{seeinMsg.releaseProcessFee||0}}</span> USDT</p>
            <p><span>用户手续费：</span><span>{{seeinMsg.buySaleProcessFee||0}}</span> USDT</p>
            </div>
...
import { atNameLists, otcOrderManaCancel, atOrderQuery, atOrderSeeIn, atOrderAudit, atOrderStatistics } from "@/api/otc_adsTreasure";
  export default {
    components: {},
    data () {
    ...
    methods: {
      orderCancelClick (id) {
        this.$prompt('备注', '确定要取消订单 ？', {
          ...
          otcOrderManaCancel(_params).then(response => {
      ...
      w2AuditClick (id) {
        ...
        this.formw2.id = id
        this.formw2.remark = ''
        this.formw2.maliciousStatus = false
        this.formw2.resultType = 0
