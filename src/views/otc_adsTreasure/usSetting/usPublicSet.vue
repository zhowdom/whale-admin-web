    ...
    <el-form :model="nLevelqueryParams" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item label="发布方向" prop="type">
          <el-select v-model="queryParams.type" clearable placeholder="请选择">
            <el-option label="全部" value=""></el-option>
            <el-option label="发布买单" :value="1"></el-option>
            <el-option label="发布卖单" :value="2"></el-option>
          </el-select>
      </el-form-item>

      <el-form-item>
        <el-button v-hasPermi="['otc:release_conf:list']" type="primary" :loading="loading" @click="searchnLevel(1)">查询</el-button>
        <el-button v-hasPermi="['otc:release_conf:storage']" type="primary" :loading="loading" icon="el-icon-plus" @click="addClick(1)">新增</el-button>
      </el-form-item>
    </el-form>
  <el-table :data="nLevelData">
    <el-table-column property="coinName" label="币种"></el-table-column>
    <el-table-column property="type" label="发布方向">
      <template scope="scope">
        <p>{{scope.row.type == '1' ? '发布买单' : '发布卖单'}}</p>
      </template>
    </el-table-column>
    <el-table-column property="singleMinNum" label="单笔最小发布数量"></el-table-column>
    <el-table-column property="singleMaxNum" label="单笔最大发布数量"></el-table-column>
    <el-table-column property="todayMaxNum" label="当日最大发布数量"></el-table-column>
    <el-table-column property="minBuySaleNum" label="最小下单数量"></el-table-column>
    <el-table-column property="maxBuySaleNum" label="最大下单数量"></el-table-column>
    ...
    <el-table-column property="enabled" label="状态">
      <template scope="scope">
          <el-switch
          v-hasPermi="['otc:release_conf:status']"
    ...
    <el-table-column label="操作">
      <template scope="scope">
        <el-button v-show="!scope.row.enabled" v-hasPermi="['otc:release_conf:storage']" type="text" @click="editClick(scope.row)">编辑</el-button>
...
<el-dialog title="" :visible.sync="dialogFormVisible" width="45%">
  ...
  <el-form-item class="requiredStyle rela" label="单笔最小发布数量" prop="singleMinNum" :label-width="formLabelWidth" style="width:500px;">
    <el-input-number @blur="((ev)=>{handleBlur(ev, 'singleMinNum')})" v-model="certifyform.singleMinNum" :step="1" :min="1" :max="1000000" step-strictly controls-position="right"></el-input-number>
  </el-form-item>
  <el-form-item class="requiredStyle rela" label="单笔最大发布数量" prop="singleMaxNum" :label-width="formLabelWidth" style="width:500px;">
    <el-input-number @blur="((ev)=>{handleBlur(ev, 'singleMaxNum')})" v-model="certifyform.singleMaxNum" :step="1" :min="1" :max="1000000" step-strictly controls-position="right"></el-input-number>
  </el-form-item>
  <el-form-item class="requiredStyle rela" label="当日最大发布数量" prop="todayMaxNum" :label-width="formLabelWidth" style="width:500px;">
    <el-input-number @blur="((ev)=>{handleBlur(ev, 'todayMaxNum')})" v-model="certifyform.todayMaxNum" :step="1" :min="1" :max="1000000" step-strictly controls-position="right"></el-input-number>
  </el-form-item>

  <el-form-item class="requiredStyle rela" label="最小下单数量" prop="minBuySaleNum" :label-width="formLabelWidth" style="width:500px;">
    <el-input-number @blur="((ev)=>{handleBlur(ev, 'minBuySaleNum')})" v-model="certifyform.minBuySaleNum" :step="1" :min="1" :max="10000" step-strictly controls-position="right"></el-input-number>
  </el-form-item>
  <el-form-item class="requiredStyle rela" label="最大下单数量" prop="maxBuySaleNum" :label-width="formLabelWidth" style="width:500px;">
    <el-input-number @blur="((ev)=>{handleBlur(ev, 'maxBuySaleNum')})" v-model="certifyform.maxBuySaleNum" :step="1" :min="1" :max="10000" step-strictly controls-position="right"></el-input-number>
  </el-form-item>
...
import { otcFBsetList, otcFBsetListAlter, otcFBsetListAlterStatus, atPropDel, atPropInfos, atPropQuery, atPropAdd, atPropStatus, atPropEdit, coinSelectNews, coinEditfb } from "@/api/otc_adsTreasure";
import {compareVersion, replaceUnitlPrefixStr} from '@/utils/index';
  export default {
    ...
    mounted () {
      ...
      this.isAuth.isPermi(['otc:release_conf:list']) && this.searchnLevel(1)
    },
    watch: {},
    methods: {
      handleBlur (ev, type) {
      ...
      saveClick () {
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            ...
            otcFBsetListAlter(_params).then(response => {
              if (+response.code === 200) {
      ...
      watchChange (val) {
        this.$confirm(`确认要执行此操作吗？`, '警告', {
          ...
          otcFBsetListAlterStatus(_params).then(response => {
            if (response.code === 200) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            otcFBsetList(_params).then(response => {
              if (response.code === 200) {
      ...
      handleSizeChangenLevel (val) {
        ...
        this.isAuth.isPermi(['otc:release_conf:list']) && this.searchnLevel()
      },
    }
  }
</script>
