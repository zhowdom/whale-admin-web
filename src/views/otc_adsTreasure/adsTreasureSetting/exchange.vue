...
<el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="200px" class="demo-ruleForm">
  <el-form-item label="单次最小兑换数量" prop="day1">
  ...
  <el-form-item label="单次最大兑换数量" prop="day2">
      ...
      query () {
        atQuery({
          ...
          if (+response.code === 200) {
            this.ruleForm = {
              day1: response.data.condition ? response.data.condition.minNum : null,
              day2: response.data.condition ? response.data.condition.maxNum : null,
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            atSave(
              {
                condition: {
                  minNum: this.ruleForm.day1,
                  maxNum: this.ruleForm.day2,
                },
