<template>
  <div class="cont_Wrapper">
    <div style="padding-bottom:15px;">
      ...
      <el-button v-hasPermi="['setting:ecology_content:save']" type="primary" icon="el-icon-plus" @click="handleAdd">新增</el-button>
    </div>
    <el-table
    :data="tableData"
    v-loading="loading"
    style="width: 100%">
      ...
      <el-table-column prop="tabName" label="tab名称" align="center" min-width="120"></el-table-column>
      ...
      <el-table-column prop="updateBy" label="操作人" align="center" min-width="120">
        ...
        <el-table-column label="操作" align="center" min-width="180">
          <template scope="scope">
            ...
            <el-button v-hasPermi="['setting:ecology_content:update']" type="primary" size="mini" @click="handleEdit(scope.row)" v-if="!scope.row.status">编辑</el-button>
            <el-button v-hasPermi="['setting:ecology_content:delete']" type="primary" size="mini" @click="handleDelete(scope.row)" v-if="!scope.row.status">删除</el-button>
          </template>
        </el-table-column>
    </el-table>
    <!--table-->
    ...
    <el-dialog custom-class="certifyForm" title="新增/编辑" :visible.sync="dialogFormVisible">
      <el-form :model="certifyform" :rules="rules" ref="ruleForm" :disabled="seeBool">
        ...
        <el-form-item label="tab名称：" :label-width="formLabelWidth" prop="tabId">
          <el-select v-model="certifyform.tabId" clearable placeholder="请选择" class="mb20">
            <el-option v-for="(its, index) in tabList" :key="index" :label="its.name" :value="its.id" :disabled="!its.status"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="" :visible.sync="dialogFormVisibleAuth" width="30%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-hasPermi="['setting:ecology_content_auth:change']" type="primary" :loading="isAuthLoading" @click="saveiSAuth">确 定</el-button>
      </div>
    </el-dialog>
...
import { ecologyContentPage, addEcologyContent, editEcologyContent, changeEcologyContent, deleteEcologyContent, ecologyTabList, dappPermissionAlter_content } from "@/api/dappManage";
import { setRoutLinkList } from "@/api/games";
  export default {
    ...
    methods: {
      ...
      // 获取tabList
      handleEcologyTabList() {
        // old need add params for web3
        ecologyTabList({id: '', clientType: 2}).then(response => {
          if (response.code === 200) {
            this.tabList = JSON.parse(JSON.stringify(response.data.list))
          } else {
            this.tabList = []
      ...
      handleSetType(row) {
        ...
        this.$confirm(`确认要${text}该配置?`, '提示', {
          ...
          changeEcologyContent(_params).then(response => {
            if (response.code === 200) {
      ...
      handleDelete(row) {
        this.$confirm('是否确定要删除该条数据?', '提示', {
          ...
          deleteEcologyContent({id: row.id}).then(response => {
            if (response.code === 200) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.certifyform.id) {
              editEcologyContent(this.certifyform).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(`保存成功`)
                  this.dialogFormVisible = false;
                  this.search(this.pageIndex);
              ...
              addEcologyContent(this.certifyform).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(`保存成功`)
                  this.dialogFormVisible = false;
                  this.search(1);
      ...
      search (pgi) {
        ...
        let _params = {
          ...
          status: '',// -1全部 0审核中 1通过 2驳回
          inviteCode: ''
        }
        this.loading = true
        ecologyContentPage(_params).then(response => {
          if (response.code === 200) {
