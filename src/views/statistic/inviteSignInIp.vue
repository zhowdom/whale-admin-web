<template>
  <div class="inviteSignInIp">
    ...
    <inviteDialog ref="inviteDialog" :isSignIn="true"></inviteDialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 查询数据
    search(pgi) {
      this.$refs["queryForm"].validate((valid, css) => {
        ...
        if (valid) {
          ...
          this.loading = true;
          getInviteeIpStatPage({ ..._params, ...this.queryParams }).then(
            (response) => {
              if (response.code === 200) {
