<template>
  <div class="shareAddManagebox">
    ...
    <el-form :model="nLevelqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <!-- <el-form-item>
        <el-button v-hasPermi="['operation:posters:save']" type="primary" :loading="loading" icon="el-icon-search" @click="searchnLevel(1)">查询</el-button>
      </el-form-item> -->
      <el-form-item>
        <el-button v-hasPermi="['operation:posters:save']" type="primary" :loading="loading" icon="el-icon-plus" @click="addClick(1)">新增</el-button>
      </el-form-item>
    </el-form>
  <el-table :data="nLevelData">
    ...
    <el-table-column property="statusType" label="状态">
      <template scope="scope">
          <el-switch
          v-hasPermi="['operation:posters:status']"
    ...
    <el-table-column label="操作">
      <template scope="scope">
        <div v-if="!scope.row.statusType">
          ...
          <el-button v-hasPermi="['operation:posters:delete']" type="text" @click="removeClick(scope.row)">删除</el-button>
        </div>
      </template>
    </el-table-column>
...
import { myPostersAdd, myPostersEdit, myPostersDel, myPostersSwitchStatus, myPostersList, takejewelCodeList, zzJoyPoorEachIssueList, h5SharePathTrackApi } from "@/api/games";
  export default {
    ...
    mounted () {
      ...
      this.isAuth.isPermi(['operation:posters:page']) && this.searchnLevel(1)
    },
    watch: {},
    methods: {
      editClick (row) {
        ...
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            let _params = {
              ...
              // platform: 1 // 平台 1:鲸丽生活 2:新APP 3:小程序 4:星火国际
            }
            this.loading = true
      ...
      handleSizeChangenLevel (val) {
        ...
        this.isAuth.isPermi(['operation:posters:page']) && this.searchnLevel()
      },
    }
  }
</script>
