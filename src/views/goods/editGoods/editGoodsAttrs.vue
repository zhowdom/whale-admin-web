<template>
  <div class="fill-goods-attr">
                ...
                >
                  ...
                  <template slot-scope="{row,$index}">
                    <el-form :model="row" :rules="rules" ref="rowForm">
                      ...
                      <!-- <el-input size="mini" v-model="row.whalePeas" placeholder="请输入销售价(Candy)"></el-input> -->
    ...
    <div class="text-center mt30 mb20">
      ...
      <el-button type="primary" @click="next" v-hasPermi="['goods:product_self:edit']">完成, 提交商品</el-button>
    </div>
    ...
    <el-dialog
      ...
      <el-form :model="attrForm" :rules="rules" ref="attrForm">
        ...
        <el-form-item label="属性名" prop="attrItemName" v-if="addKey==1">
          <el-input placeholder="请输入内容" v-model="attrForm.attrItemName" />
        </el-form-item>
        <el-form-item label="属性值" prop="attrItemValue" v-if="addKey==2">
          <el-input placeholder="请输入内容" v-model="attrForm.attrItemValue" />
        </el-form-item>
      </el-form>
...
import {uuid, getUrlName, strSym} from '@/utils/index';
import {goodsAttributeList} from '@/api/goods/goodsAbout';
import {attributeItem, productEdit, attrByCateId, descHtml} from '@/api/goods/index';
...
export default {
  ...
  deactivated () {
    ...
    this.deletedSku = [];
    this.addedVal = [];
    this.uploadConf.fileList = [];
    this.uploadConfLong.fileList = [];
  },
  mounted () {
  ...
  methods: {
    ...
    async setPart3Data () {
      ...
      let {items, description, refer, images, primaryImage, activityImage, couponId, payWay} = session.get('part3Info');
      ...
      // 如果已有图片隐藏上传选块
      if (activityImage) {
        const name = getUrlName(activityImage);
        const item = {name, url: activityImage};
        this.uploadConfLong.fileList.push(item);
      ...
      if (this.images.length) {
        this.images.map(url => {
          ...
          const item = {name, url};
          this.uploadConf.fileList.push(item);
        });
        if (this.images.length >= 5) {
          this.switchUploader('uploadAlbum', 'none');
        }
      };
      ...
      items.map(pl => {
        ...
        uniquekey = md5(JSON.stringify(propObj)).toString();
        this.$set(pl, 'uniquekey', uniquekey);
      });
    ...
    handleExceed (files, fileList) {
      let total = this.uploadConf.limit;
      let curLen = files.length;
      let hadLen = fileList.length;
      let curTotal = curLen + hadLen;

      this.$message.warning(`
        当前限制选择 ${total} 个文件，本次选择了 ${curLen} 个文件，
        共选择了 ${curTotal} 个文件
      `);
    },
    // 移除上传
    ...
    async productEditReq(data) {
      ...
      try {
        ...
        const {code, message} = await productEdit({
          id,
          ...part1,
          ...part2,
          ...part3,
    ...
    next() {
      ...
      const {items: initItems} = session.get('part3Info');
      ...
      const items = this.tableData.map(({
        ...
        skuNo, stock, supplyPrice, virSales, waring, id,
      }) => {
        const item = {
          editMarketPrice,
          editPrice,
          ...
          waring,
          // virSales: '',      // 虚拟销量
          productId,            // 商品的ID, 编辑必须传入
          enable: true,         // true不删除,false删除 编辑必须传入
          id,                   // sku的ID, 编辑必须传入
        };
        if (item.id === undefined) Reflect.deleteProperty(item, 'id');
      ...
      // 2. 将被删除的sku与items合并
      initItems.map(pl => {
        let isEnable = items.some(cl => cl.id && cl.id == pl.id);
        if (!isEnable) {
          pl['enable'] = false;
          items.push(pl);
        };
      ...
      if (result) this.productEditReq(part3);
    },
    // 清空缓存
    clearCache () {
