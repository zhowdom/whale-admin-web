      ...
      <div class="body">
        <el-form :model="mainForm" :rules="rules" ref="mainForm" label-width="120px" class="custom-form">
          ...
          <el-form-item label="商品品牌" prop="brandId">
            <el-col :span="17">
              <el-select class="wd90" popper-class="brands" v-model="mainForm.brandId" placeholder="请选择商品品牌" clearable>
                <el-option
        ...
        <el-form :model="subForm" :rules="rules" ref="subForm" label-width="130px" class="custom-form">
          ...
          <el-form-item label="商品关键词">
            ...
            <div class="input-tips">
              点击可增加/删除关键词；<br>
              关键词有两个功能，一是可以作为站内关键词查询，在前台搜索框输入关键词后，能够搜索到该商品；二是作为搜索引擎收录使用.</div>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    setEditPart2Data () {
      ...
      } = session.get('part2Info');

      autoUpTiming = autoUpTiming ? new Date(autoUpTiming).getTime() : null;
      ...
      this.subForm = {
        services, remark, keywords, recommended, commission, purchaseMin, purchaseMax, day, hour, minute, showTip,
        showSoldOut
      };
    },
    // 获取上一步选择的商品分类
    ...
    async templsRequest () {
      try {
        const {data} = await templsAll({type: 2});
    ...
    // 校验表单数据
    validateForms () {
      const isBrandHidden = this.brands.some(({brandId, showStatus}) => !showStatus && brandId == this.mainForm.brandId);
      if (isBrandHidden) return this.$message.error('当前选择的商品品牌已被隐藏无法使用，请修改！');
