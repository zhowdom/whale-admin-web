<template>
  <div class="add-templ">
    <el-card class="box-card" shadow="always">
      <el-form :model="mainForm" :rules="rules" ref="mainForm" class="mt40">
        <el-form-item label="模板名称:" prop="name" :inline-message="true">
          <el-input class="input-class" v-model="mainForm.name" placeholder="请输入模板名称"></el-input>
        </el-form-item>

        <el-form-item label="是否包邮:" prop="shippingType" :inline-message="true">
            ...
            <el-table border size="mini" :data="specs" style="width: 100%">
              ...
              <el-table-column prop="startPoint" align="center" :label="`首${tabSetUnit}`">
                <template slot-scope="{row}">
                  <el-input v-input-filter:int2="{key:'startPoint',row}" v-model.lazy="row.startPoint" placeholder="请输入"></el-input>
                </template>
              </el-table-column>
...
import {templSave} from '@/api/goods/index';
  ...
  methods: {
    ...
    async templSaveRequest () {
      try {
        ...
        const {code, message} = await templSave(reqData);
        if (code == 200) {
