        ...
        <div class="class-wrap">
          ...
          <el-card class="main-class">
            ...
            <template v-if="level1.length">
              <div v-for="{categoryName, iconUrl, categoryId} in level1" :key="categoryId" :class="{item:true,active:pid==categoryId}" @click="selectLevel1(categoryName, categoryId)">
          ...
          <el-card class="sub-class">
            ...
            <template v-if="level2.length">
              <div v-for="{categoryName, iconUrl, categoryId} in level2" :key="categoryId" :class="{item:true,active:cid==categoryId}" class="item" @click="selectLevel2(categoryName, categoryId)">
                <div class="label">{{categoryName}}</div>
                <div class="img mr20">
                  <img v-show="iconUrl" :src="iconUrl" alt="">
                </div>
...
import {paramsSave, paramsUpdate, paramsDetail, noPageCategory} from '@/api/goods/index';
  ...
  methods: {
    ...
    async noPageCategoryRequest (parentId = null) {
      try {
        ...
        const {data: {list}} = await noPageCategory({parentId, queryHidden});
