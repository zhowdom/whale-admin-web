  ...
  <div class="userpower_Wrapper">
      ...
      <el-form :model="queryParams" ref="queryForm" :inline="true" class="mytopformstyle">
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="queryParams.status" clearable placeholder="请选择">
            ...
            <el-option label="确认成功" :value="6"></el-option>
            <el-option label="执行失败" :value="7"></el-option>
          </el-select>
        </el-form-item>
        ...
        <!-- <el-form-item label="失败原因" prop="type">
          <el-select v-model="queryParams.type" clearable placeholder="请选择">
            <el-option label="全部" value=""></el-option>
            <el-option label="链上已到账" :value="1"></el-option>
            <el-option label="链上未有记录" :value="2"></el-option>
          </el-select>
        </el-form-item> -->
        <el-form-item label="" style="border:none;">
          <el-button v-hasPermi="['wallet:coin:transaction:waitConfirm:page']" :loading="loading" type="primary" icon="el-icon-search" @click="search(1)">查询</el-button>
    ...
    style="width: 100%">
      ...
      <el-table-column
        ...
        label="操作">
          <template scope="scope">
            <el-button v-hasPermi="['wallet:coin:transaction:waitConfirm:page']" type="primary" size="mini" @click="hadIncomeClick(scope.row)">链上已到账</el-button><br>
            <el-button v-hasPermi="['wallet:coin:transaction:waitConfirm:page']" style="margin-top:10px;" type="primary" size="mini" @click="resetClick(scope.row)">重置状态</el-button>
    ...
    <el-dialog
      title="链上已到账，手动确认已完成"
      :visible.sync="visiblefaillogSuccess"
      width="30%">
      <span style="padding-right:10px;float:left;">TxID:</span><el-input type="textarea" :rows="3" style="display:inline-block;width:339px;" v-model.trim="transactionHashEntered" placeholder="请输入链上转账哈希"></el-input>
      <p>（提交后，提现的订单状态转流转为【已完成】状态）</p>
      <span slot="footer" class="dialog-footer">
        <el-button @click="visiblefaillogSuccess = false">取 消</el-button>
        <el-button type="primary" :disabled="transactionHashEntered == ''" @click="hadIncomeClickRun">确 定</el-button>
      </span>
    </el-dialog>
    <!--重置-->
    <el-dialog
      title="确定重置"
        :visible.sync="visiblefaillogFail"
        width="30%">
        <span>提交后，提现的订单状态转流转为【待审核】状态，需要在【充提明细】列表处，重新走审核流程</span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="visiblefaillogFail = false">取 消</el-button>
          <el-button type="primary" @click="resetClickRun">确 定</el-button>
        </span>
      </el-dialog>
  </div>
</template>
...
<script>
import { xhgjSpark_wdlist, xhgjSpark_wdlistv2, widSuccess, widFail, xhgjSpark_audit, xhgjSpark_auditBatch, xhgjSpark_platWalletSendCode } from "@/api/xhgjSpark_api";
...
import {userAreaPhone} from '@/api/pointsMgmt/starTasks';
  export default {
    data () {
      return {
        transactionHashEntered: '',
        rowFail: {},
        rowSuccess: {},
        visiblefaillogSuccess: false,
        visiblefaillogFail: false,
        formitpAudit: {
          auditStatus: 1,
          phoneOrder: '1',
          auditMessage: '',
    ...
    methods: {
      ...
      hadIncomeClick (row) {
        this.transactionHashEntered = ''
        this.visiblefaillogSuccess = true
        this.rowSuccess = Object.assign({}, row)
      },
      hadIncomeClickRun () {
        widSuccess({
          transactionHash: this.transactionHashEntered,
          sn: this.rowSuccess.sn
        }).then(response => {
            if (+response.code === 200) {
            this.$message({
                message: '操作成功',
                ...
                onClose: () => {
                  this.search()
                  this.visiblefaillogSuccess = false
                }
              })
          } else {
            this.$message.error(response.message)
      ...
      resetClickRun () {
        widFail({
          transactionHash: this.rowFail.txnHash,
          sn: this.rowFail.sn
        }).then(response => {
            if (+response.code === 200) {
            this.$message({
      ...
      resetClick (row) {
        this.visiblefaillogFail = true
        this.rowFail = Object.assign({}, row)
      },
      ...
      search (pgi) {
        this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            xhgjSpark_wdlistv2(_params).then(response => {
                if (response.code === 200) {
