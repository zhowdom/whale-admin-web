<template>
  <div class="tab-content">
    <div class="mb30">
      <el-button v-hasPermi="['whale:exchange_rule:save']" icon="el-icon-plus" type="primary" @click="handleAdd()">新增</el-button>
    ...
    style="width: 100%">
      <el-table-column label="闪兑币种" align="center" min-width="120">
      ...
      <!-- <el-table-column prop="status" label="状态" align="center" min-width="120">
        <template slot-scope="{row}">
          <el-switch v-hasPermi="['whale:exchange_rule:change']" v-model="row.status" @change="handleSetType(row)"></el-switch>
        </template>
      </el-table-column> -->
      <el-table-column label="操作" align="center" width="180">
        <template scope="scope">
          <el-button v-hasPermi="['whale:exchange_rule:update']" type="primary" size="mini" @click="handleEdit(scope.row)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!--table-->
    <!--新增/编辑dialog-->
    <el-dialog custom-class="certifyForm" title="新增/编辑闪兑配置" :visible.sync="dialogFormVisible" :width="'600px'">
      <el-form :model="certifyform" :rules="rules" ref="ruleForm">
        <el-form-item label="闪兑币种：" :label-width="formLabelWidth" prop="fromName">
          <el-col :span="11">
        ...
        <el-form-item label="闪兑汇率：" :label-width="formLabelWidth" prop="exchangeRateType">
...
<script>
import { exchangeList, exchangePage, exchangeSave, exchangeUpdate, exchangeChange } from "@/api/xhgjSpark_api/daiCoinSetting.js";
export default {
  data() {
    const validateFromTo = (rule, value, callback) => {
      if(value === '') {
        callback(new Error('请选择闪兑币种'));
  ...
  methods: {
    ...
    search (pgi) {
      ...
      this.loading = true
      exchangePage(_params).then(response => {
        if (response.code === 200) {
    ...
    handleSetType(row) {
      ...
      this.$confirm(`确认要${text}该配置?`, '提示', {
        ...
        exchangeChange({id: row.id}).then(response => {
          if (response.code === 200) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(this.certifyform.id) {
            ...
            exchangeUpdate(_params).then(response => {
              if (response.code === 200) {
                this.msgSuccess(`保存成功`)
            ...
            exchangeSave(_params).then(response => {
              if (response.code === 200) {
                this.msgSuccess(`保存成功`)
                this.dialogFormVisible = false;
                this.search(1);
