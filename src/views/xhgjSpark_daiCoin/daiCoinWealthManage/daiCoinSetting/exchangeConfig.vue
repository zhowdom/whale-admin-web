<template>
    <div class="tab-content">
      <div class="mb30">
        <el-button v-hasPermi="['whale:exchange_dh_setting:save']" icon="el-icon-plus" type="primary" @click="handleAdd()">新增</el-button>
      </div>
      <!--table-->
      <el-table
      :data="tableData"
      ...
      style="width: 100%">
        <el-table-column prop="ruleId" label="兑换币对" align="center" min-width="120">
          <template slot-scope="{row}">
            <span v-if="row.ruleId">{{exchangeSettingObj[row.ruleId]}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="singleMinAmount" label="单笔最小兑换额" align="center" min-width="120"></el-table-column>
        <el-table-column prop="singleMaxAmount" label="单笔最大兑换额" align="center" min-width="120"></el-table-column>
        <el-table-column prop="dayMaxAmount" label="当日最大可兑换" align="center" min-width="120"></el-table-column>
        ...
        <el-table-column prop="coinStatus" label="代币状态" align="center" min-width="120">
          <template slot-scope="{row}">
        ...
        <el-table-column prop="status" label="状态" align="center" min-width="120">
          <template slot-scope="{row}">
            <el-switch v-hasPermi="['whale:exchange_dh_setting:change']" v-model="row.status" @change="handleSetType(row)"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="180">
          <template scope="scope">
            <el-button v-hasPermi="['whale:exchange_dh_setting:update']" type="primary" size="mini" @click="handelEdit(scope.row)" :disabled="scope.row.status">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <!--table-->
      <!--新增/编辑dialog-->
      <el-dialog custom-class="certifyForm" title="兑换配置" :visible.sync="dialogFormVisible" :width="'700px'">
        <el-form :model="certifyform" :rules="rules" ref="ruleForm">
          <el-form-item label="兑换币种：" :label-width="formLabelWidth" prop="ruleId">
          ...
          <el-form-item label="单笔最小兑换额：" :label-width="formLabelWidth" prop="singleMinAmount">
          ...
          <el-form-item label="单笔最大兑换额：" :label-width="formLabelWidth" prop="singleMaxAmount">
          ...
          <el-form-item label="当日最大可兑换：" :label-width="formLabelWidth" prop="dayMaxAmount">
          ...
          <div style="text-indent: 40px;">
            <div>假设：</div>
            <p class="text-muted">(1) 兑换 candy - usdt</p>
            <p class="text-muted">(2) 兑换100 candy 手续费配置 ： 1% ， Gas fee   配置为 1 USDT</p>
            <p class="text-muted">(3) 1 candy : 1 usdt  = 1：3.43</p>
            <p class="text-muted">(4)当前等级 LV2 ，转赠逻辑手续费 35%</p>
            <div>计算：</div>
            <p class="text-muted">(1)实际使用Candy 来USDT： 100</p>
            <p class="text-muted">(2)实际扣减  = 使用兑换数量  + 转赠逻辑手续费 + 配置的手续费</p>
            <p class="text-muted" style="text-indent: 117px;">=  100 + 100  * 35%+ 100*1% = 136 candy</p>
            <p class="text-muted">(3)兑换扣减Gasfee = 1 USDT</p>
            <p class="text-muted">(4)得到 USDT = 100 × 3.43 = 343</p>
            <p class="text-muted">Gasfee 为 Candy 的 ，其扣减额受转赠逻辑影响</p>
  ...
  <script>
  import { exchangeConfigList, exchangeConfigPage, exchangeConfigSave, exchangeConfigUpdate, exchangeConfigChange } from "@/api/xhgjSpark_api/daiCoinSetting.js";
  export default {
    data() {
    ...
    methods: {
      ...
      search (pgi) {
        ...
        this.loading = true
        exchangeConfigPage(_params).then(response => {
          if (response.code === 200) {
      ...
      handleSetType(row) {
        ...
        this.$confirm(`确认要${text}该配置?`, '提示', {
          ...
          exchangeConfigChange({id: row.id}).then(response => {
            if (response.code === 200) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.certifyform.id) {
              exchangeConfigUpdate(_params).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(`保存成功`)
              ...
              exchangeConfigSave(_params).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(`保存成功`)
                  this.dialogFormVisible = false;
                  this.search(1);
