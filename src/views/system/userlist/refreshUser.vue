<template>
  <div class="userlistwapper">
    ...
    <el-form :model="queryParams" ref="queryForm" :rules="rulesTop" :inline="true">
      ...
      <el-form-item label="最后登陆时间" prop="regtimeLogin">
        <el-date-picker
          v-model="queryParams.regtimeLogin"
          type="daterange"
          ref="dateRangeLogin"
          value-format="timestamp"
          range-separator="至"
      ...
      <el-form-item>
        <el-button v-hasPermi="['member:member:filter']" type="primary" :loading="loading" icon="el-icon-search" @click="search(1, 'clickQuery')">查询</el-button>
    ...
    <div>
      <!-- <el-checkbox style="margin-top:10px;" v-model="loginPwdFilter">登陆密码相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="payPwdFilter">支付密码相同</el-checkbox> -->
      <el-checkbox style="margin-top:10px;" v-model="directlyUnderNameFilter">直属上级真实姓名相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="directlyUnderPhoneFilter">直属上级手机号相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="whalePeasFilter">Candy相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="dedicateWorthFilter">荣耀值相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="shareWorthFilter">分享值相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="singleReadWorthFilter">阅读值相同</el-checkbox>
      <el-checkbox style="margin-top:10px;" v-model="activeWorthFilter">活跃值相同</el-checkbox>
    </div>
    ...
    <el-table highlight-current-row
      ...
      <el-table-column
        ...
        <template slot-scope="{row}">
          {{row.registeredAt ? $dayjs(row.registeredAt).format('YYYY-MM-DD HH:mm:ss') : '-'}}
        </template>
      </el-table-column>
      <el-table-column
        header-align="center"
      ...
      <!-- <el-table-column
        ...
        label="支付密码">
        <template scope="scope">
          <div>{{scope.row.payPwd}}</div>
        </template>
      </el-table-column> -->
      <el-table-column
        header-align="center"
        align="center"
        prop="directlyUnderRealName"
        label="直属上级真实姓名">
        <template scope="scope">
          <div>{{scope.row.directlyUnderRealName}}</div>
        </template>
      </el-table-column>
      <el-table-column
        width="120"
        header-align="center"
        align="center"
        prop="directlyUnderRealPhone"
        label="直属上级手机号">
        <template scope="scope">
          <div>{{scope.row.directlyUnderRealPhone}}</div>
        </template>
      </el-table-column>
      <el-table-column
        width="120"
      ...
      <el-table-column
        ...
        label="荣耀值">
        <template scope="scope">
          <div>{{scope.row.dedicateWorth}}</div>
        </template>
      </el-table-column>
      <el-table-column
        width="120"
      ...
      <el-table-column
        ...
        header-align="center"
        align="center"
        prop="singleReadWorth"
        label="阅读值">
        <template scope="scope">
          <div>{{scope.row.singleReadWorth}}</div>
        </template>
      </el-table-column>
      <el-table-column
        width="120"
...
<script>
import { searchUserListFilter, someoneCertifyResult, searchUserList, getCertificate, upload, refuseOrApprove, save, statusChange, statusBatchChange } from "@/api/system/userlist";
...
export default {
  data() {
    ...
    return {
      loginPwdFilter: false,
      payPwdFilter: false,
      directlyUnderNameFilter: false,
      directlyUnderPhoneFilter: false,
      whalePeasFilter: false,
      dedicateWorthFilter: false,
      shareWorthFilter: false,
      singleReadWorthFilter: false,
      activeWorthFilter: false,
      export2Excel,
      ctx: this,
      beforeWatchChange: 1,
      curHandleFlags: '',
  ...
  created () {
    // 今日之前3个月
    // this.queryParams.regtime = [new Date().getTime() - 3600 * 1000 * 24 * 90, new Date().getTime()]
    // 默认今日
    this.queryParams.regtime = [new Date().getTime(), new Date().getTime()]
  },
  mounted () {
    ...
    search (pgi, acitionType) {
      this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            // 校验注册时间、最后登陆时间
            if (!this.queryParams.regtime || (this.queryParams.regtime && !this.queryParams.regtime[0] && !this.queryParams.regtime[1])) {
              this.$message({
                message: '请选择注册时间',
                type: 'warning'
              })
            ...
            if (this.queryParams.regtime && this.queryParams.regtime[1] - this.queryParams.regtime[0] > 3600 * 24 * 1000 * 100) {
              this.$message({
                message: '注册时间范围不能超过100天',
                type: 'warning'
              })
              return false
            }
            if (this.queryParams.regtimeLogin && this.queryParams.regtimeLogin[1] - this.queryParams.regtimeLogin[0] > 3600 * 24 * 1000 * 100) {
              this.$message({
                message: '最后登陆时间范围不能超过100天',
                type: 'warning'
              })
              return false
            }
            ...
            //   cityId: this.queryParams.cityName || '',
            ...
            // }
            let _params = {
              registeredAtStart: this.queryParams.regtime && this.$dayjs(this.queryParams.regtime[0]).format('YYYY-MM-DD'),
              registeredAtEnd: this.queryParams.regtime && this.$dayjs(this.queryParams.regtime[1]).format('YYYY-MM-DD'),
              lastLoginAtStart: this.queryParams.regtimeLogin && this.$dayjs(this.queryParams.regtimeLogin[0]).format('YYYY-MM-DD'),
              lastLoginAtEnd: this.queryParams.regtimeLogin && this.$dayjs(this.queryParams.regtimeLogin[1]).format('YYYY-MM-DD'),
              pageSize: this.pageSize,
              currentPage: this.pageIndex,
              // loginPwdFilter: this.loginPwdFilter ? 1 : 0,
              // payPwdFilter: this.payPwdFilter ? 1 : 0,
              directlyUnderNameFilter: this.directlyUnderNameFilter ? 1 : 0,
              directlyUnderPhoneFilter: this.directlyUnderPhoneFilter ? 1 : 0,
              whalePeasFilter: this.whalePeasFilter ? 1 : 0,
              dedicateWorthFilter: this.dedicateWorthFilter ? 1 : 0,
              shareWorthFilter: this.shareWorthFilter ? 1 : 0,
              singleReadWorthFilter: this.singleReadWorthFilter ? 1 : 0,
              activeWorthFilter: this.activeWorthFilter ? 1 : 0
            }
            this.loading = true
            // searchUserList(_params).then(response => {
            searchUserListFilter(_params).then(response => {
              if (response.code === 200) {
