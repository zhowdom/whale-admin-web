  ...
  <div class="userlistwapper">
    ...
    <el-table highlight-current-row
      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <div v-else-if="+scope.row.payType === 3">余额</div>
        </template>
      </el-table-column>
      <el-table-column
        width="110"
        ...
        <template scope="scope">
          <div>
          <div v-if='+scope.row.authStatus === -1'>未认证</div>
          <div v-else-if='+scope.row.authStatus === 1'>认证成功</div>
          <div v-else-if='+scope.row.authStatus === 2'>认证失败</div>
          <div v-else-if='+scope.row.authStatus === 4'>待审核</div>
          </div>
        </template>
      </el-table-column>
      <!-- <el-table-column
      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <div class="certifyResbox" v-hasPermi="['member:rpAuth:auth']">
            <div @click="clickCertifyResult(scope.row.memberId)" style="color:#1890ff;text-decoration: underline;">查看</div>
            <div v-if="+scope.row.authStatus === 2" @click="drawbackClick(scope.row)" style="color:#1890ff;text-decoration: underline;">退款</div>
          </div>
        </template>
      </el-table-column>
    ...
    <el-dialog custom-class="certifyForm" title="实名认证审核" :visible.sync="dialogFormVisible">
      ...
      <el-form :model="certifyform" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="退款记录：" :label-width="formLabelWidth">
          <ul style="padding:0;">
            <li v-for="(item, index) in drawbackMoneyWarnInfosArr" :key="index" style="color:red;padding-top:9px;">
              {{item.createdAt ? $dayjs(item.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-'}}执行退款操作
            </li>
          </ul>
        </el-form-item>
      </el-form>
...
<script>
import { someoneCertifyResult, searchUserList, getCertificate, upload, refuseOrApprove, save, statusChange, statusBatchChange, drawbackMoneyList, drawbackMoney, drawbackMoneyWarnInfos } from "@/api/system/userlist";
...
export default {
  ...
  methods: {
    drawbackMoneyWarnInfosRun (memberId) {
      // drawbackMoneyWarnInfosArr
      drawbackMoneyWarnInfos(
        {
          memberId,
          returnType: 2
        }
        ...
        if (+response.code === 200) {
          this.drawbackMoneyWarnInfosArr = response.data.list || []
        } else {
          this.drawbackMoneyWarnInfosArr = []
        }
      })
    },
    drawbackClickRun (row) {
      drawbackMoney(
        {
    ...
    drawbackClick (row) {
      this.$confirm('确认退款后，认证费用将原路退回，将清空历史提交的认证信息，认证状态由认证失败变为未认证', {
        type: 'warning',
        showClose: false,
...
    },
    ...
    search (pgi, acitionType) {
      this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            drawbackMoneyList(_params).then(response => {
              if (response.code === 200) {
    ...
    clickCertifyResult (id) {
      ...
      this.drawbackMoneyWarnInfosRun(id)
    },
