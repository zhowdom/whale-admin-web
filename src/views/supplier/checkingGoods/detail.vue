<template>
  <div class="checking-goods-detail">
    <div class="base part0">
      <div class="body">
        <el-row class="info-item" :gutter="20">
          <el-col class="item-label" :span="4">状态:</el-col>
          <el-col :span="15">
            {{status==1?'新建':status==3?'上架审核成功':status==5?'上架审核失败':status==6?'下架审核成功':status==7?'停用':'-'}}
          </el-col>
        </el-row>
        <el-row class="info-item" :gutter="20" v-if="status==5">
          <el-col class="item-label" :span="4">审核失败备注:</el-col>
          <el-col class="text-block" :span="15">{{info.auditRemark||'-'}}</el-col>
        </el-row>
        <el-row class="info-item" :gutter="20" v-if="status==3||status==6">
          <el-col class="item-label" :span="4">审核成功备注:</el-col>
          <el-col class="text-block" :span="15">{{info.auditRemark||'-'}}</el-col>
        </el-row>
      </div>
    </div>
