<template>
  <div class="seeInfosEachissueBox bbStatisticBox" style="padding:20px;">
    ...
    <el-form :model="nsqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item label="期数" prop="period">
        <el-input
          v-model.trim="nsqueryParams.period"
          placeholder="请输入"
          clearable
          size="small"
          style="width: 130px"
      ...
      <el-form-item label="手机号" prop="memberPhone">
        <el-input
          v-model.trim="nsqueryParams.memberPhone"
          placeholder="请输入"
          clearable
          size="small"
          style="width: 150px"
      ...
      <el-form-item label="邮箱" prop="memberEmail">
        <el-input
          v-model.trim="nsqueryParams.memberEmail"
          placeholder="请输入"
          clearable
          size="small"
          style="width: 150px"
      ...
      <el-form-item label="触发返还时间" prop="redbacktime">
        <el-date-picker
      ...
      <el-form-item label="平台流水号" prop="tradeNo">
        <el-input
      ...
      <!-- <el-form-item label="第三方流水号" prop="thirdTradeNo">
        <el-input
  ...
  <el-table :data="nLevelData" @selection-change="handleSelectionChange">
    ...
    <el-table-column property="memberPhone" label="手机号" width="120"></el-table-column>
    <el-table-column property="memberEmail" label="邮箱" width="120"></el-table-column>
    ...
    <el-table-column property="tradeNo" label="平台流水号"></el-table-column>
    <!-- <el-table-column property="thirdTradeNo" label="第三方流水号"></el-table-column> -->
    <el-table-column property="returnPrice" label="返还金额（USDT）"></el-table-column>
    <el-table-column property="payType" label="返还渠道">
      <template scope="scope">
    ...
    <!-- <el-table-column property="status" label="状态">
      <template scope="scope">
        ...
        <p v-else-if="+scope.row.status === 2">审核失败</p>
        <p v-else-if="+scope.row.status === 3">转账中</p>
        <p v-else-if="+scope.row.status === 4">转账失败</p>
        <p v-else-if="+scope.row.status === 5">转账成功</p>
        <p v-else-if="+scope.row.status === 0">-</p>
      </template>
    </el-table-column> -->
    <el-table-column property="transferAt" label="触发返还时间">
      <template slot-scope="{row}">
        {{row.transferAt ? $dayjs(row.transferAt).format('YYYY-MM-DD HH:mm:ss') : '-'}}
      </template>
    </el-table-column>
    
    <el-table-column property="updatedBy" label="操作人"></el-table-column>
    <el-table-column property="updatedAt" label="操作时间">
      <template slot-scope="{row}">
        {{row.updatedAt ? $dayjs(row.updatedAt).format('YYYY-MM-DD HH:mm:ss') : '-'}}
      </template>
    </el-table-column>
    <el-table-column property="auditErrorReason" label="审核失败原因"></el-table-column>
...
<script>
import { blindBoxTFAudit, blindBoxTFList, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
    ...
    methods: {
      ...
      searchnLevel (pgi) {
        ...
        datetime3 = this.nsqueryParams.redbacktime3
        ...
        let _params = {
          ...
          memberPhone: this.nsqueryParams.memberPhone,
          memberEmail: this.nsqueryParams.memberEmail,
          ...
          period: this.nsqueryParams.period,
          transferEndAt: datetime ? this.$dayjs(datetime[1]).format('YYYY-MM-DD') : '',
          transferResponseEndAt: datetime3 ? this.$dayjs(datetime3[1]).format('YYYY-MM-DD') : '',
          transferResponseStartAt: datetime3 ? this.$dayjs(datetime3[0]).format('YYYY-MM-DD') : '',
          transferStartAt: datetime ? this.$dayjs(datetime[0]).format('YYYY-MM-DD') : '',
        }

        this.loading = true
        blindBoxTFList(_params).then(response => {
          if (response.code === 200) {
            this.nLevelData = response.data.pageRes.list
            this.totalPagenLevel = response.data.pageRes.total
            this.loading = false
            this.$emit('copyMsgFromChild', {
              allInAmount: response.data.returnRecordPageStatisticsResp.allInAmount || 0,
              auditErrorAmount: response.data.returnRecordPageStatisticsResp.auditErrorAmount || 0,
              transferErrorAmount: response.data.returnRecordPageStatisticsResp.transferErrorAmount || 0,
              transferSuccessAmount: response.data.returnRecordPageStatisticsResp.transferSuccessAmount || 0,
              waitAuditAmount: response.data.returnRecordPageStatisticsResp.waitAuditAmount || 0,
            })
          } else {
            this.nLevelData = []
