<template>
  <div class="withdrawSet">
    ...
    <div class="enterVal" style="text-align:center;">
      <span>每笔提现手续费占提现金额的比例：</span>
      <el-input-number :precision="2" v-model="sumbitForm.poundageRatio" controls-position="right" @change="handleChange" :min="0" :max="100" @blur="((ev)=>{handleBlur(ev, 'poundageRatio')})"></el-input-number><span class="unit">%</span>
      <el-checkbox v-model="sumbitForm.enabledPoundageRatio">禁用</el-checkbox>
    </div>
...
import { saveWithdrawSetting, queryWithdrawSetting, queryWithdrawList, withdrawCount, withdrawAudit  } from "@/api/wealth/withdrawSet";
  export default {
    ...
    methods: {
      /** 
       * 盲盒收益提现配置：
       * checkBox的选中与补选中的状态判断：
       * 后端返回false -> 代表勾选了｜返回true -> 代表没有勾选
       */
      saveClick (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            _params.enabledPoundageRatio = this.sumbitForm.enabledPoundageRatio ? false : true
            _params.withdrawalType = 2
        ...
        queryWithdrawSetting({
          ...
          if (response.code === 200) {
            this.sumbitForm = Object.assign(response.data, {
              ...
              enabledPoundageRatio: response.data.enabledPoundageRatio ? false : true,
            })
          } else {
            this.$message.error(response.message)
          }
