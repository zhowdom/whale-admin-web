<template>
  <div class="redBackDetailsBox">
    <el-form :model="queryParams" ref="queryForm" :rules="rules" :inline="true">
      ...
      <el-form-item label="提现状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择">
          ...
          <el-option label="转账给失败" :value="5"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-form-item v-show="false" label="是否已退回" prop="refundStatus">
    ...
    <el-form-item label="提现类型">
      <el-select v-model="queryParams.withdrawalType" placeholder="请选择">
        <el-option label="全部" value=""></el-option>
        <el-option label="常规提现" :value="1"></el-option>
        <el-option label="盲盒提现" :value="2"></el-option>
      </el-select>
    </el-form-item>
    ...
    <el-table-column
      ...
      align="center">
      <template scope="scope">
        <p v-if="+scope.row.withdrawalType === 1">常规提现</p>
        <p v-else-if="+scope.row.withdrawalType === 2">盲盒提现</p>
        <p v-else>-</p>
      </template>
    </el-table-column>
    <el-table-column
    ...
    <el-table-column
      ...
      align="center">
      <template scope="scope">
        <p v-if="scope.row.refundStatus === true">是</p>
        <p v-else-if="scope.row.refundStatus === false">否</p>
        <p v-else>-</p>
      </template>
    </el-table-column>
    <el-table-column
    ...
    <el-table-column
      ...
      align="center">
      <template scope="scope">
        <el-button v-hasPermi="['financial:withdraw:withdrawAudit']" :disabled="+scope.row.status !== 1" type="primary" size="mini" @click="auditClick(scope.row)">审核</el-button>
      </template>
    </el-table-column>
        ...
        this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            let _params = {
              ...
              withdrawalType: this.queryParams.withdrawalType
            }
            this.loading = true
