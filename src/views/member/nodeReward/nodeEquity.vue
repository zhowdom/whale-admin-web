<template>
  <div class="tab-content">
    <div class="mb30 text-muted ft14">
      <p>节点权益 ： 是指成为指定节点后，有权益领取相应的激励包 至 【我的卡券】，且达到相应的达标条件，才可领取 ，满足领取条件后，即可激活。</p>
    </div>
    <div class="task-table">
      ...
      style="width: 100%">
        ...
        <el-table-column prop="vipLevel" label="达标条件" align="center" min-width="120">
          <template slot-scope="{row}">
            {{row.vipLevel ? vipLevelObj[row.vipLevel] : '暂无'}}
          </template>
        </el-table-column>
        <el-table-column prop="payInfo" label="领取条件" align="center" min-width="120">
          <template slot-scope="{row}">
            <span v-if="row.payInfo && row.payInfo.length">{{row.payInfo[0].price}} {{row.payInfo[0].payWay}}</span>
            <span v-else>--</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="激励包" align="center" min-width="120"></el-table-column>
        <el-table-column prop="term" label="协议期限(天)" align="center" min-width="120"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="120">
          <template scope="scope">
            <el-button v-hasPermi="['task:node_setting_equity:update']" type="primary" size="mini" @click="handelEdit(scope.row)" :disabled="scope.row.status">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--table-->
    ...
    <!--新增/编辑dialog-->
    <el-dialog custom-class="certifyForm" title="节点权益配置" :visible.sync="dialogFormVisible" :width="'600px'">
      <el-form :model="certifyform" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="领取条件：" :label-width="formLabelWidth" prop="payInfo">
          <el-col :span="20">
            <el-input v-model.number="certifyform.payInfo[0].price"></el-input>
          </el-col>
...
<script>
import { nodeEquityPage, editNodeEquity, changeNodeEquityReward, rewardList, asteriskTypes } from "@/api/member/nodeConfig";

  export default {
    components: {},
    data () {
      const validateCondition = (rule, value, callback) => {
        if(!this.certifyform.payInfo[0].price) {
      ...
      return {
        ...
        certifyform: {
          id: '',
          payInfo: [{ payWay: "USDT", price: null }],
          taskInfoId: '',
          vipLevel: '',
          term: null
        },
    ...
    methods: {
      ...
      search (pgi) {
        ...
        this.loading = true
        nodeEquityPage(_params).then(response => {
          if (response.code === 200) {
      ...
      handelEdit(row) {
        ...
        if(!row.payInfo || row.payInfo.length <= 0) {
          row.payInfo = [{ payWay: "USDT", price: null }];
        }
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let _params = {
              ...
              payInfo: [{ payWay: "USDT", price: this.certifyform.payInfo[0].price }],
              taskInfoId: this.certifyform.taskInfoId,
              vipLevel: this.certifyform.vipLevel,
              term: this.certifyform.term
            }
            this.submitLoading = true;
            editNodeEquity(_params).then(response => {
              if (response.code === 200) {
                this.msgSuccess(`保存成功`)
