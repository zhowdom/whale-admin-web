<template>
  ...
  <el-table :data="nLevelData">
    ...
    <el-table-column property="downLimit" label="默认底注（Candy）"></el-table-column>
...
<script>
import { takejewelCodeList, zzJoyPoorEachIssueList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            zzJoyPoorEachIssueList(_params).then(response => {
