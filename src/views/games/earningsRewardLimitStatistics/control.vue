<template>
  <div class="seeInfosEachissueBox">
    ...
    <el-form :model="nLevelqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item>
        <el-button v-hasPermi="['blindBox:member_control_score_record:all_page']" type="primary" :loading="loading" icon="el-icon-search" @click="searchnLevel(1)">查询</el-button>
      </el-form-item>
    </el-form>
    <p style="color:#3c3c3c;font-size:13px;">
      管制分：<span style="font-weight:bold;color:orange;">{{controlScoreTotal}}</span>
    </p>
  <el-table :data="nLevelData" border>
    ...
    <!-- <el-table-column property="buyNum" label="每份可参与"></el-table-column> -->
    <el-table-column property="controlScore" label="管制分"></el-table-column>
    <!-- <el-table-column property="buyNum" label="可参与"></el-table-column> -->
...
<script>
import { earningsRewardLimitStatistics_control, blindBoxDetailsProfit, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            earningsRewardLimitStatistics_control(_params).then(response => {
              if (response.code === 200) {
                this.nLevelData = response.data.pageResult.list
                this.controlScoreTotal = response.data.controlScore || 0
                this.totalPagenLevel = response.data.pageResult.total
                this.loading = false
              } else {
                this.nLevelData = []
                this.controlScoreTotal = 0
