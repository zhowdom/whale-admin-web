<template>
  <div class="seeInfosEachissueBox">
    ...
    <p style="color:#3c3c3c;font-size:13px;">
      累计任务券（张）：<span style="font-weight:bold;color:orange;">{{giveNumTotal}}</span>
    </p>
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="giveNum" label="数量（张）"></el-table-column>
    <el-table-column property="giveType" label="任务券类型">
      <template scope="scope">{{+scope.row.giveType === 1 ? '任务券' : 'VIP'}}</template>
    </el-table-column>
...
<script>
import { blindBoxRuleAddCoupons, earningsRewardLimitStatistics_taskVip, blindBoxDetailsProfit, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
