<template>
  ...
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="withdrawTotal" label="可提现（USDT）"></el-table-column>
    ...
    <el-table-column property="settlementDate" label="更新时间">
...
<script>
import { earningsRewardLimitStatistics_profitDetail, blindBoxDetailsProfit, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            earningsRewardLimitStatistics_profitDetail(_params).then(response => {
              if (response.code === 200) {
