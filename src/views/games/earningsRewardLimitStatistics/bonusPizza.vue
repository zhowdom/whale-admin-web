<template>
  <div class="seeInfosEachissueBox">
    ...
    <p style="color:#3c3c3c;font-size:13px;">
      总Pizza：  <span  style="font-weight:bold;color:orange;">{{coinValue}}</span>
    </p>
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="blindBoxType" label="类型">
      <template scope="scope">
        ...
        <p v-else-if="+scope.row.blindBoxType === 2">积分销毁专场</p>
    ...
    <el-table-column property="singleUsableNum" label="每份Pizza"></el-table-column>
    <el-table-column property="buyNum" label="抢购份数"></el-table-column>
    <el-table-column property="coinValue" label="赠送Pizza"></el-table-column>
...
<script>
import { miniBlindBox_bonusJDTListQuery, earningsRewardLimitStatistics_profitDetail, blindBoxDetailsProfit, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
        ...
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            miniBlindBox_bonusJDTListQuery(_params).then(response => {
              if (response.code === 200) {
                this.nLevelData = response.data.pageResult.list
                this.coinValue = response.data.coinValue || 0
                this.totalPagenLevel = response.data.pageResult.total
                this.loading = false
              } else {
                this.nLevelData = []
