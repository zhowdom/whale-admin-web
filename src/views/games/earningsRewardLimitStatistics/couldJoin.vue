<template>
  <div class="seeInfosEachissueBox">
    ...
    <el-form :model="nLevelqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item>
        <el-button v-hasPermi="['blindBox:product_control:all_page']" type="primary" :loading="loading" icon="el-icon-search" @click="searchnLevel(1)">查询</el-button>
      </el-form-item>
    </el-form>
    <p style="color:#3c3c3c;font-size:13px;">
      可参与：<span style="font-weight:bold;color:orange;">{{usableValueTotal}}</span>
    </p>
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="singleUsableIntegral" label="每份可参与"></el-table-column>
    <!-- <el-table-column property="controlScore" label="管制分"></el-table-column> -->
    <el-table-column property="usableValue" label="Candy"></el-table-column>
...
<script>
import { earningsRewardLimitStatistics_couldJoin, blindBoxDetailsProfit, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            earningsRewardLimitStatistics_couldJoin(_params).then(response => {
              if (response.code === 200) {
                this.nLevelData = response.data.pageRes.list
                this.usableValueTotal = response.data.usableValue || 0
                this.totalPagenLevel = response.data.pageRes.total
                this.loading = false
              } else {
                this.nLevelData = []
                this.usableValueTotal = 0
