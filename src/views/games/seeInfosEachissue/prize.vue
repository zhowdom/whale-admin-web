  ...
  <div class="prizesAwardBox" style="padding: 20px;">
    ...
    <el-table-column
      ...
      <template slot-scope="{row}">
        ...
        <!-- <el-button v-hasPermi="['game:prizeDistribution:addAddress']" v-if="row.address" @click="addOrEditAddressClick(row, 'edit')" type="text">编辑</el-button>
        <el-button v-hasPermi="['game:prizeDistribution:addLogisticsNum']" @click="addPostCodeClick(row)" type="text">添加快递单号</el-button>
...
import regionData from '@/utils/provinceCity'
  export default {
    ...
    mounted () {
      ...
      this.isAuth.isPermi(['game:prizeDistribution:listByPage']) && this.search()
    },
    watch: {},
    methods: {
        ...
        this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            let _params = {
              ...
              currentPage: this.pageIndex,
              pageSize: this.pageSize,
              gameWhalePeasSeizeTreasureId: this.$route.query.id
            }
            this.loading = true
