<template>
  <div class="prizesAwardBox" style="padding: 20px;">
    <el-form :model="queryParams" :rules="rules" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        <el-button v-hasPermi="['game:lottery:pool:getByPage']" type="primary" :loading="loading" icon="el-icon-search" @click="search(1)">查询</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table-column
        ...
        <template slot="header" slot-scope="scope">
          ...
          class="item"
          effect="dark"
          content="1. 每期奖池,不得超过奖池上限
          2. 每期奖池 = 上期未分配奖金 + 奖池默认底注 + 每期注入奖金"
          placement="top">
    ...
    <el-table-column
        ...
        <template slot="header" slot-scope="scope">
          ...
          class="item"
          effect="dark"
          content="未分配奖金,是指某个等级的奖励没有用户中奖,故没有分配."
          placement="top">
          <i class="el-icon-info"></i>
          </el-tooltip>
        </template>
    ...
    <el-table-column
      ...
      align="center">
      <template slot-scope="{row}">
      <p @click="nLevelPrizeInfosClick(row, 2)" style="color:#1890ff;text-decoration:underline;cursor:pointer;">{{row.levelNum2}}</p>
      </template>
    </el-table-column>
    <el-table-column
      prop="levelWhalePeas3"
      label="三等奖奖金(Candy)"
      align="center">
    </el-table-column>
    <el-table-column
      ...
      align="center">
      <template slot-scope="{row}">
      <p @click="nLevelPrizeInfosClick(row, 3)" style="color:#1890ff;text-decoration:underline;cursor:pointer;">{{row.levelNum3}}</p>
      </template>
    </el-table-column>
    <el-table-column
      prop="levelWhalePeas4"
      label="四等奖奖金(Candy)"
      align="center">
    </el-table-column>
    <el-table-column
      ...
      align="center">
      <template slot-scope="{row}">
      <p @click="nLevelPrizeInfosClick(row, 4)" style="color:#1890ff;text-decoration:underline;cursor:pointer;">{{row.levelNum4}}</p>
      </template>
    </el-table-column>
    <el-table-column
      prop="levelWhalePeas5"
      label="五等奖奖金(Candy)"
      align="center">
    </el-table-column>
    <el-table-column
      ...
      align="center">
      <template slot-scope="{row}">
      <p @click="nLevelPrizeInfosClick(row, 5)" style="color:#1890ff;text-decoration:underline;cursor:pointer;">{{row.levelNum5}}</p>
      </template>
    </el-table-column>
    <el-table-column
      prop="levelWhalePeas6"
      label="六等奖奖金(Candy)"
      align="center">
    </el-table-column>
    <el-table-column
      ...
      align="center">
      <template slot-scope="{row}">
        <p @click="nLevelPrizeInfosClick(row, 6)" style="color:#1890ff;text-decoration:underline;cursor:pointer;">{{row.levelNum6}}</p>
      </template>
    </el-table-column>
    <el-table-column
      prop="status"
      ...
      align="center">
      <template slot-scope="{row}">
        <p v-if="+row.status === 0">待开奖</p>
        <p v-else-if="+row.status === 1">已开奖</p>
      </template>
    </el-table-column>
...
<el-dialog :title="nLevelPrizeTitle" :visible.sync="nLevelPrizesVisible">
  <el-form :model="nLevelqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item>
        <el-button v-hasPermi="['game:lottery:pool:getByLevelPage']" type="primary" :loading="nLevelLoading" icon="el-icon-search" @click="searchnLevel(1)">查询</el-button>
      </el-form-item>
    </el-form>
  <el-table :data="nLevelData" v-loading="nLevelLoading">
...
<script>
import { prizesAwardQueryList, prizesAwardExport, prizesAwardAddPostCode, prizesAwardAddOrEditAddress, prizesAwardDetails, takejewelWinQuery, takejewelWinSet,takejewelWinCodeChange,takejewelWinCodeCancel, zzJoyWinnerPage, zzJoyWinnerLevelPage } from "@/api/games";
  ...
  export default {
    ...
    methods: {
      nLevelPrizeInfosClick (row, n) {
        ...
        this.nLevelPrizeTitle = `${n}等奖管理【第${row.periodsNum}期】(只展示真实中奖数据)`
      ...
      search (pgi) {
        this.$refs['queryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            zzJoyWinnerPage(_params).then(response => {
              if (response.code === 200) {
