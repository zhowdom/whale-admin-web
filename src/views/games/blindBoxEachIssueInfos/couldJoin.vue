<template>
  ...
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="singleUsableIntegral" label="每份可参与（Candy）"></el-table-column>
    <el-table-column property="usableValue" label="可参与（Candy）"></el-table-column>
...
<script>
import { coudlJoinRcdPage, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            coudlJoinRcdPage(_params).then(response => {
              if (response.code === 200) {
