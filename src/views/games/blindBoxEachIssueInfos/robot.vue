<template>
  <div class="seeInfosEachissueBox">
    ...
    <el-form :model="nLevelqueryParams" :rules="nLevelrules" ref="nLevelqueryForm" :inline="true">
      ...
      <el-form-item>
        <el-button v-hasPermi="['blindBox:product_series:robotInfo']" type="primary" :loading="loading" icon="el-icon-search" @click="searchnLevel(1)">查询</el-button>
      </el-form-item>
    </el-form>
  <el-table :data="nLevelData" border>
    ...
    <el-table-column property="robotRatio" label="机器人占比">
      <template scope="scope">
        <p v-if="scope.row.robotRatio === null || scope.row.robotRatio === ''">-</p>
        <p v-if="+scope.row.robotRatio === 0">{{scope.row.robotRatio}}%</p>
        <p v-else>{{scope.row.robotRatio}}</p>
      </template>
    </el-table-column>
    <el-table-column property="robotShareNum" label="机器人抢购份数"></el-table-column>
    <el-table-column property="userShareNum" label="真实抢购份数"></el-table-column>

    <!-- <el-table-column property="num" label="已解封盒机">
      <template scope="scope">
          <p @click="bugFirstClick(scope.row)" style="color:#1890ff;cursor:pointer;text-decoration:underline;">{{scope.row.num || 0}}</p>
        </template>
    </el-table-column>
    <el-table-column property="time" label="解封时间">
...
<script>
import { robotRcdPage, blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            robotRcdPage(_params).then(response => {
              if (response.code === 200) {
