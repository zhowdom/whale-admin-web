<template>
  ...
  <el-pagination
    ...
    <!--通用型Dialog-s-->
    <el-dialog :title="`盲盒实物(期数：${curClickRowData.periods})`" :visible.sync="cmbTableVisible" width="66%">
...
<script>
import { blindBoxDetailsGetByLockPage, blindBoxPurchaseInfoItemQuery, takejewelCodeList, zzJoyPoorTotalSwitchList } from "@/api/games";
  export default {
    components: {},
    data () {
      ...
      searchnLevel (pgi) {
        this.$refs['nLevelqueryForm'].validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            blindBoxDetailsGetByLockPage(_params).then(response => {
              if (response.code === 200) {
