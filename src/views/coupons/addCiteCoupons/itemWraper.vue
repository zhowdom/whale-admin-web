    ...
    <div class="goods-drawer">
      <div :class="`drawer-wraper ${this.hasBlur?'':'no-blur'}`">
        ...
        <template v-if="drawerType==1">
          <goods ref="goods" :max-count="maxCount"/>
        </template>

        <!-- 指定分类 -->
        <template v-if="drawerType==2">
          <categories ref="categories" :max-count="maxCount"/>
        </template>

        <!-- 指定供应商 -->
        <template v-if="drawerType==3">
          <suppliers ref="suppliers" :max-count="maxCount"/>
        </template>

        <!-- 指定自营店 -->
        <template v-if="drawerType==4">
          <shops ref="shops" :max-count="maxCount"/>
        </template>
      </div>
    </div>
    <el-divider></el-divider>
