    ...
    <div class="init-table">
      <div class="table-header">
        <div class="table-tips">
          最多可以添加{{maxCount}}个供应商  已选中
          &nbsp;<span class="text-danger">{{addedTable.length}}</span>&nbsp;个供应商
        </div>
...
<script>
import {sellerList, seller500List} from '@/api/coupons';
...
export default {
  ...
  methods: {
    ...
    async sellerListReq () {
      try {
        ...
        const {code, message, data: {list, pageSize, total}} = await sellerList({
          ...
          orderItemList: [{asc: false, column: "createdAt"}],
        });
        if (code == 200) {
          this.initTable = list;
    ...
    // 选中的供应商列表
    async sellerSelectedListReq () {
      try {
        if (!this.itemIds?.length) return;
        const {code, message, data: {list}} = await seller500List({
          status: 1,
          sellerIds: this.itemIds.join(','),
          orderItemList: [{asc: false, column: "createdAt"}],
        });
        if (code == 200) {
