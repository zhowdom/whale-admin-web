...
<script>
import {categoryTree, categoryList, category500List} from '@/api/coupons';
...
export default {
  ...
  methods: {
    ...
    // 商品分类分页选中的商品
    async categorySelectedListReq () {
      try {
        if (!this.itemIds?.length) return;
        const {code, message, data: {list}} = await category500List({
          categoryIds: this.itemIds.join(','),
        });
        if (code == 200) {
