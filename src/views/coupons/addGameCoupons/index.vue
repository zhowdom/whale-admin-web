<template>
  <div class="add-giving-coupons">
    <el-card class="wrap-card" shadow="always" header="添加游戏优惠券">
      <el-form :model="mainForm" :rules="rules" ref="mainForm" label-width="200px" class="mt40">

        <el-form-item label="到券方式：">
          <div class="text-muted">游戏赠送</div>
        </el-form-item>

        <el-form-item label="优惠券类型：" prop="type">
          <el-select class="input-class" v-model="mainForm.type" placeholder="请选择优惠券类型" @change="changeTypeHandle">
        ...
        <template v-if="mainForm.type==0">
          <el-form-item label="适合用户：" prop="phone">
            <el-input class="input-class" placeholder="请输入手机号" v-model="mainForm.phone">
        ...
        <!-- <template style="display:none!important;" v-if="mainForm.type==1">
          <el-form-item style="display:none!important;" label="总发行量：" prop="stock">
        ...
        <!-- <template style="display:none!important;" v-if="mainForm.type==1">
          <el-form-item style="display:none!important;" label="开始发放时间：" prop="givingAt">
            <el-date-picker
        ...
        <el-form-item>
          <div class="form-btns">
            ...
            <el-button type="primary" v-hasPermi="['coupon:game:add']" @click="submitForm">保存</el-button>
...
import {memberSearch, addGivingCoupon} from '@/api/coupons/game';
...
export default {
  ...
  data () {
    return {
      ...
      mainForm: {
        name: '',                             // 优惠券名称
        phone: '',                            // 选择用户查询的手机号
        type: 1,                              // 优惠券类型 0:指定个人赠送优惠券 1:指定用户群赠送优惠券
        timeRange: [],                        // 有效期范围
        limitNumber: 1,                       // 每人限领数
        ...
        effectiveType: 1,                     // 有效期类型 0:日期范围 1:固定天数
        expireDay: '',                        // 有效期天数
        givingAt: '',                         // 优惠卷发放时间
  ...
  beforeRouteEnter (to, from, next) {
    ...
    next(vm => {
      if (path == '/promotion/addGameCoupon') vm.fromList = true;
      return true;
    });
  },
  ...
  methods: {
    ...
    async addGivingCouponReq (cb) {
      try {
        ...
        reqDate = {
          ...
          name, type, limitNumber, couponAmount, applyGoodsIds,
          useType, fullReductionAmount, effectiveType, expireDay, applyGoodsType, remark,
        };
        ...
        const {code, data} = await addGivingCoupon(reqDate);
        if (code == 200) {
    ...
    submitForm() {
      this.$refs.mainForm.validate(valid => {
        if (valid) {
          let ids = [];
          const {applyGoodsType} = this.mainForm;
          if (applyGoodsType) {
            ids = this.$refs.witems.handleCheckIds();
            if (!ids) return false;
          };
          this.mainForm.applyGoodsIds = ids;
          const cb = this.loadLoading();
          setTimeout(() => this.addGivingCouponReq(cb), 800);
        } else {
          return false;
        }
      });
