<template>
  <div class="order-shipping-list-wrap" v-loading="showPost">
    ...
    <div v-if="id" key="1">
      ...
      <div class="form-btns">
        <el-button type="primary" v-hasPermi="['operation:order:batchDeliver']" @click="confirmReturn">发货</el-button>
      </div>
    </div>
        ...
        <el-row :gutter="10">
          <el-col :span="24">
            <el-form-item class="fr mr0">
              ...
              <el-button type="primary" v-hasPermi="['operation:order:batchDeliver']" @click="handleMultOrder">批量发货</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      ...
      <el-table border row-key="orderNo" ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" label="操作" width="100">
          <template slot-scope="{row,$index}">
            <el-button class="mb5" size="mini" type="primary" v-hasPermi="['operation:order:batchDeliver']" @click="handleSingleOrder(row, $index)">发货</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import {deliverPage, batchDeliver} from '@/api/order/list';

export default {
  name: 'OrderShippingList',
  props: ['id', 'memberOPhone', 'range', 'orderOName', 'sellerId', 'type'],
  data () {
    return {
      mms,
      pager: {
  ...
  methods: {
    ...
    async deliverPageReq () {
      try {
        ...
        const {code, message, data: {list, pageSize, total}} = await deliverPage(reqData);
    ...
    handleSingleOrder (row, index) {
      let reqData = [];
      let logisticsName;
      let {logisticsCode, logisticsNum, orderId, orderNo} = row;
    ...
    handleMultOrder () {
      ...
      // 组合数据
      this.multipleSel.map(({logisticsCode, logisticsNum, orderId, orderNo}) => {
        let logisticsName;
        this.mms.some(({name, code}) => {
          if (code == logisticsCode) {
            logisticsName = name;
    ...
    confirmReturn () {
      ...
      // 组合数据
      this.tableData.map(({logisticsCode, logisticsNum, orderId, orderNo}) => {
        let logisticsName;
        this.mms.some(({name, code}) => {
          if (code == logisticsCode) {
            logisticsName = name;
