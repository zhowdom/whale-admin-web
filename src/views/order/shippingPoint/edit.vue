<template>
  <div class="edit-shipping-point-wrap">
...
import {shippingPointUpdate, shippingPointDetail} from '@/api/order/shippingPoint';

export default {
  name: 'EditShippingPoint',
  computed: {
    citysData() {
      citys.forEach(e => {
  ...
  methods: {
    ...
    // 获取详情
    async shippingPointDetailReq () {
      try {
        const {code, message, data} = await shippingPointDetail({id: this.id});
        if (code == 200) {
          let {
            deliverName,
            deliverUserName,
          ...
          this.mainForm.area = areaCode ? [provinceCode, cityCode, areaCode] : [provinceCode, cityCode];
        } else {
          this.$message.error(message);
        };
      } catch (error) {
