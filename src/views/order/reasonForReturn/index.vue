<template>
  <div class="reason-for-return-wrap">
...
export default {
  ...
  methods: {
    ...
    // 批量删除操作
    batchDelHandle () {
      if (!this.multipleSel.length) return this.$message.error('请选择批量删除的退款原因！');
      let ids = this.multipleSel.map(({id}) => id);

      this.$confirm('确定要删除这些退款原因？', {
        type: 'warning',
    ...
    // 加载loading
    loadLoading () {
      const target = this.$refs.return.$el.firstChild;
