      ...
      <el-form :model="mainForm" :rules="rules" ref="mainForm" label-width="200px" class="mt40">
        ...
        <el-form-item>
          <div class="form-btns">
            ...
            <el-button type="primary" v-hasPermi="['setting:popup_advert_v2:edit', 'setting:popup_advert_v2:save']" @click="submitForm">保存</el-button>
          </div>
        </el-form-item>
      </el-form>
    </el-card>
...
import {advertPOP_list, advertPOP_Detail, advertPOP_del, advertPOP_add, advertPOP_alter, bigOrSmallBannerPositionQuery, setRoutLinkList} from '@/api/games';
  ...
  methods: {
    ...
    async advertDetailReq () {
      try {
        ...
        let {code, message, data: {
          name, position, online, remark, params, clickUrl, imgUrl, linkType, startTime, endTime, type, jumpLinkId
        }} = await advertPOP_Detail({id: this.id});
    ...
    async saveOrUpdateReq (cb) {
      try {
        ...
        const reqFn = this.isEdit ? advertPOP_alter : advertPOP_add;
        const reqData = this.isEdit ? {id, ...formatData} : formatData;
        const {code, message} = await reqFn(
          Object.assign(reqData, {appPackageType: 3})
        );
        if (code == 200) {
    ...
    goBack2List () {
      ...
      this.$store.dispatch('tagsView/delView', curView).then(() => {
        this.$nextTick(() => {
          this.$router.replace({path: 'popAdsMgmtNewv2', query: {tabActive: 'mini'}}); // 返回列表页
        });
      });
    },
    // 重置功能
