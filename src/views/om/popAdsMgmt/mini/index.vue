        ...
        <el-row :gutter="20">
          <el-col :span="24">
            ...
            <el-form-item class="fr mr0">
              <el-button size="small" class="mr20" type="primary" @click="onSearch" v-hasPermi="['setting:popup_advert:page']">查询</el-button>
              <el-button size="small" type="primary" @click="handleTabBtn({}, 'Add')" v-hasPermi="['setting:popup_advert:save']">添加弹框广告</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      ...
      <el-table border :data="tableData" style="width: 100%" @sort-change="sortFunction">
        ...
        <el-table-column align="center" label="操作" width="170">
          <template slot-scope="{row}">
            <el-button size="mini" type="primary" v-hasPermi="['setting:popup_advert:edit']" @click="handleTabBtn(row, 'Edit')">编辑</el-button>
            <el-button v-if="!row.online" size="mini" type="danger" v-hasPermi="['setting:popup_advert:del']" @click="delPopAds(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    async advertPageReq () {
      try {
        ...
        const {code, message, data: {list, pageSize, total}} = await advertPage({
          ...this.mainForm,
          pageSize: this.pager.size,
          currentPage: this.pager.curPage,
          applicationPackageType: 3
        });
    ...
    async advertDeleteReq (id) {
      try {
        const {code, message} = await advertDelete({id});
        if (code == 200) {
          this.advertPageReq();
    ...
    handleTabBtn(row, type) {
      ...
      const path = type == 'Add' ? 'addPopAdsMini' : 'editPopAdsMini';
