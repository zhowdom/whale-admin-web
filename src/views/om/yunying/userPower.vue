  ...
  <div class="userpower_Wrapper">
        <div class="topshow">
            ...
            <el-form-item label="手机号" prop="phone">
                <el-input maxlength="11" v-model="queryParams.phone" placeholder="请输入手机号" clearable></el-input>
            </el-form-item>
            <el-form-item label="邮箱" prop="email">
                <el-input v-model="queryParams.email" placeholder="请输入手机号" clearable></el-input>
            </el-form-item>
            <el-form-item label="" style="border:none;">
                ...
                <el-button v-hasPermi="['agora:member:prerogative:save']" type="primary" icon="el-icon-plus" @click="addAuditPowerClick">添加用户/特权</el-button>
            </el-form-item>
            </el-form>
        </div>
        <!--table-->
        ...
        style="width: 100%">
            ...
            <!-- <el-table-column
              ...
              label="操作">
              <template scope="scope">
                <el-button v-hasPermi="['member:prerogative:update']" :disabled="scope.row.statusType === 1" type="primary" size="mini" @click="editAuditPowerClick(scope.row)">编辑</el-button>
                <el-button v-hasPermi="['member:prerogative:audit']" :disabled="scope.row.statusType === 1" type="primary" size="mini" @click="auditBatchClick(scope.row)">{{scope.row.statusType === 1 ? '已审核' : '审核'}}</el-button>
                <el-button v-hasPermi="['member:prerogative:delete:batch']" :disabled="scope.row.statusType === 1" type="primary" size="mini" @click="deletBatchClick(scope.row)">移除特权</el-button>
              </template>
            </el-table-column> -->
        </el-table>
        <!--table-->
        <div style="padding: 20px 0" v-show="false">
          <el-button v-hasPermi="['member:prerogative:audit']" :disabled="multipleSelection.length <= 0 || hadValidedUser_STATUS" @click="toggleSelectionAudit()">批量审核</el-button>
          <el-button v-hasPermi="['member:prerogative:delete:batch']" :disabled="multipleSelection.length <= 0 || hadValidedUser_STATUS" @click="toggleSelectionDelet()">批量移除</el-button>
        </div>
        <el-pagination
        ...
        <el-dialog width="85%" title="权益设置新增" :visible.sync="auditSettingVisible">
          <el-form :model="auditSettingform_for_ADD" ref="addAuditPowerForm">
            ...
            <el-form-item label="专属特权配置" :label-width="formLabelWidth" class="notP customStyle">
                ...
                <ul>
                  ...
                  <li class="clearfix" v-for="(item, index) in auditTypeMapData" :key="index + 'plus'" v-show="item.visible && tabPosition === 'plus' && markEachAuditReduceOrPlus[item.msg]">
                    ...
                    <div class="auditCenter fl" style="padding-left:10px;" v-if="item.type !== 7">
                      ...
                      <el-input-number v-else-if="item.type === 17" :disabled="!item.editable" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 18" :disabled="!item.editable" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 19" :disabled="!item.editable" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 20" :disabled="!item.editable" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 21" :disabled="!item.editable" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                  ...
                  <li class="clearfix" v-for="(item, index) in auditTypeMapData" :key="index + 'reduce'" v-show="item.visible && tabPosition === 'reduce' && !markEachAuditReduceOrPlus[item.msg]">
                    ...
                    <div class="auditCenter fl" style="padding-left:10px;" v-if="item.type !== 7">
                      ...
                      <el-input-number v-else-if="item.type === 11 || item.type === 12" :step="1" :min="0.000001" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 14" :step="1" :min="0.01" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 15" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
                      <el-input-number v-else-if="item.type === 16" :step="1" :min="0.001" :precision="item.decimalScale" v-model="auditSettingform_for_ADD[`num_${index}`]" controls-position="right" @change="handleChange" @blur="((ev)=>{handleBlur(ev, index)})"></el-input-number>
              ...
              <el-button type="primary" :loading="btnloading" @click="saveAuditPower">确 定</el-button>
            </el-form-item>
          </el-form>
        </el-dialog>
        ...
        <el-dialog width="85%" title="权益设置修改" :visible.sync="auditSettingVisible_edit">
          <el-form :model="auditSettingform_for_edit" ref="editAuditPowerForm">
            ...
            <el-form-item label="专属特权配置" :label-width="formLabelWidth" class="notP customStyle">
                ...
                <ul>
                  ...
                  <li class="clearfix" v-for="(item, index) in auditTypeMapData" :key="index + 'plus'" v-show="item.visible && tabPosition === 'plus' && markEachAuditReduceOrPlus[item.msg]">
                    ...
                    <div class="auditCenter fl" style="padding-left:10px;" v-if="item.type !== 7">
                      ...
                      <el-input-number step-strictly v-if="item.type === 3" :step="1" :min="0" :max="50" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
                      <el-input-number v-else-if="item.type === 4" :step="1" :min="0.01" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
                      <el-input-number step-strictly v-else-if="item.type === 5" :step="1" :min="1" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
                      <el-input-number v-else-if="item.type === 6" :step="1" :min="0.001" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
                      <el-input-number step-strictly v-else-if="item.type === 8 || item.type === 9 || item.type === 10" :step="1" :min="0" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
                      <el-input-number v-else :min="0.000001" :precision="item.decimalScale" v-model="auditSettingform_for_edit[`num_${index}`]" controls-position="right" @change="handleChange"></el-input-number>
              ...
              <el-button type="primary" @click="saveAuditPower_edit">确 定</el-button>
            </el-form-item>
          </el-form>
        </el-dialog>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleBlur (ev, index) {
        ...
        if (ev.target.ariaValueNow === 'undefined') this.auditSettingform_for_ADD[`num_${index}`] = ev.target.value = ev.target.ariaValueMin
      },
      handleChange (value) {
        this.$forceUpdate()
