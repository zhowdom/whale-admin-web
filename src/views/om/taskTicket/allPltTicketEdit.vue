      ...
      <el-col :span="12" :offset="7">
        <div class="form-btns">
          ...
          <template v-if="type=='Add'">
            <el-button type="primary" v-hasPermi="['integral:task:save:global']" @click="submitForm">保存</el-button>
          </template>
          <template v-else>
            <el-button type="primary" v-hasPermi="['integral:task:update:global']" @click="submitForm">保存</el-button>
          </template>
        </div>
      </el-col>
    </el-row>
...
import {saveGlobal, updateGlobal, typeList, activeTypeList, taskInfo} from '@/api/om/task';
  ...
  data () {
    return {
      ...
      rules: {
        ...
        activeStartAt: [
          { required: true, message: '开始时间设定不能为空', trigger: 'blur' }
        ],
        giveDeadline: [
          { required: true, message: '赠送期限不能为空', trigger: 'blur' },
    ...
    async submitData () {
      try {
        ...
        if (activeStartAt && giveDeadline) {
          ...
          // 计算 + 格式化结束时间
          tempStart = new Date(activeStartAt);
