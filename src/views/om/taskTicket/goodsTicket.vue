      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-form ref="taskForm" inline :model="taskForm" label-width="90px">
            ...
            <el-form-item>
              <el-button type="primary" @click="onSearch" v-hasPermi="['integral:task:product:page']">查询</el-button>
            </el-form-item>
            <el-form-item class="fr mr0">
              <el-button type="primary" @click="onAdd" v-hasPermi="['integral:task:save:product']">新增</el-button>
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
      ...
      <el-table border :data="tableData" style="width: 100%" @sort-change="sortFunction">
        ...
        <el-table-column prop="statusType" align="center" label="状态" width="120">
          <template slot-scope="{row}">
            <!-- 状态 0 未启用 1启用 -->
            <el-switch v-hasPermi="['integral:task:status:change:product']" @change="statusChange(row)" class="table-switch" :value="row.statusType" :width="80" active-text="已启用" inactive-text="已禁用" :active-value="1" :inactive-value="0"></el-switch>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作" width="250">
          <template slot-scope="{row}">
            <el-button v-if="showBtns(row, 'Edit')" size="mini" type="primary" v-hasPermi="['integral:task:update:product']" @click="handleTabBtn(row, 'Edit')">编辑</el-button>
            <el-button v-if="showBtns(row, 'Detail')" size="mini" type="warning" v-hasPermi="['integral:task:product:page']" @click="handleTabBtn(row, 'Detail')">查看</el-button>
            <el-button v-if="showBtns(row, 'Recording')" size="mini" type="info" v-hasPermi="['integral:task:product:page']" @click="handleRecordBtn(row, 'Recording')">修改记录</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import {pageGlobal, pageGoods, modifyLog, activeTypeList, statusChangeGlobal, statusChangeGoods} from '@/api/om/task';
