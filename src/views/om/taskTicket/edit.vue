      ...
      <el-col :span="12" :offset="7">
        <div class="form-btns">
          ...
          <template v-if="type=='Add'">
            <el-button type="primary" v-hasPermi="['integral:task:save:prerogative']" @click="submitForm">保存</el-button>
          </template>
          <template v-else>
            <el-button type="primary" v-hasPermi="['integral:task:update:prerogative']" @click="submitForm">保存</el-button>
          </template>
        </div>
      </el-col>
    </el-row>
...
import {taskInfo, savePrerogative, updatePrerogative} from '@/api/pointsMgmt/starTasks';
  ...
  methods: {
    ...
    async submitData () {
      try {
        ...
        const reqFn = this.type == 'Add' ? savePrerogative : updatePrerogative;
        const taskAttribute = {...this.uiForm1, ...this.uiForm2, ...this.uiForm3};
        const receiveCondition = this.mainForm.receiveCondition.replace('VIP0', '');

        const reqData = this.type == 'Add'
          ? {...this.mainForm, taskAttribute, receiveCondition}
          : {...this.mainForm, taskAttribute, receiveCondition, id: this.id};
