  ...
  <div class="card1p">
    ...
    <div class="title">
      <div class="desc" v-show="isShowLeft">
        <div class="text">模块配置-卡片1图</div>
      </div>
    </div>
    <div class="body" v-show="isShowLeft">
      <template v-for="(formitem) in formsArr">
        <div class="item" :key="formitem.fid">
          <el-form :model="formitem" ref="atomform" :key="formitem.fid" :rules="rules" class="custom-from">
            ...
            <el-form-item label-width="60px" required prop="src" label="图标: " :rules="rules.src">
              <el-upload
                ...
                :http-request="(file) => handleRequestItem(file, 'uploadItem')"
                :on-remove="(file, fileList) => handleRemove(file, fileList, 'uploadItem')"
              >
                <i class="el-icon-upload"></i>
              </el-upload>
            </el-form-item>
            <div class="divider-tips">配置链接: </div>
            <el-form-item required label-width="100px" prop="type" label="类型: " :rules="rules.type">
              <el-select v-model="formitem.type" placeholder="请选择" clearable>
                <el-option v-for="({type,name},cpindex) in cpsType" :key="cpindex" :label="name" :value="type"></el-option>
              </el-select>
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        type: [
          {required: true, message: 'cps类型不能为空', trigger: ['blur', 'change']},
        ],
        src: [
  ...
  methods: {
    ...
    switchUploader (ref, arg) {
      this.$nextTick(() => {
        const uploader = this.$refs[ref][0].$el.querySelector('.el-upload');
    ...
    handleRemove(file, fileList, ref) {
      this.$refs[ref][0].uploadFiles.splice(0, 1);
      this.switchUploader(ref, 'inline-block');
    },
    // 重写提交方法
    handleRequestItem ({file}, ref) {
      const hasValid = this.beforeuploadItem(file);
      if (hasValid) {
        this.uploadRequset(file, ref);
      } else {
        this.file = null;
        this.$refs[ref][0].uploadFiles.splice(0, 1);
      }
    ...
    // 发起图片上传请求
    async uploadRequset (file, ref) {
      this.switchUploader(ref, 'none');
      try {
        ...
        if (status == 200 && statusCode == 200) {
          ...
          this.$set(this.formsArr[0], 'src', img);
    ...
    setDataHandle(data) {
      ...
      this.$nextTick(() => {
        const src = this.formsArr[0]['src'];
        const uploadFiles = this.$refs.uploadItem[0].uploadFiles;
        if (src) {
          uploadFiles.length = 0;
          uploadFiles.push({url: src, uid: Date.now()});
          this.switchUploader('uploadItem', 'none');
        }
      });
    },
    // 重置表单
